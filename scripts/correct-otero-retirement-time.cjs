  ...
  try {
    const result = await sql.begin(async tx => {
      ...
      if (!before || before.date !== day || before.clientAccount !== 'CLI-0156' || before.status !== 'Completado' ||
          ...
          JSON.stringify(before.technicianIds?.map(String)) !== '["1786104063545"]' || before.sourceTaskId !== taskId ||
          ...
          destination.tasks.filter(matches).length !== 1) throw Object.assign(new Error('El estado cambió'), { code: 'TARGET_CHANGED' })
      ...
      const next = { ...before, time: '11:00', scheduledTime: '11:00' }
      const state = structuredClone(current)
      state.history = state.history.map(r => r.id === id ? next : r)
      const correct = target => {
        for (const team of target.teams || []) {
          team.tasks = (team.tasks || []).map(task => matches(task) ? { ...task, time: '11:00', scheduledTime: '11:00' } : task)
        }
      }
      correct(state.agenda.weekly[day])
      if (state.agenda.date === day) correct(state.agenda)
      ...
      if (apply) {
        ...
        await appendOperationalAudit(tx, [{ id: randomUUID(), at: new Date().toISOString(), user: { id: actor.id, name: actor.name, role: actor.role },
          action: 'Corrigió horario de baja completada de 19:00 a 11:00 mediante Codex (autorización expresa)', entity: 'Servicio / historial', entityId: id,
          before, after: next }])
      }
