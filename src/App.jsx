...
function AgendaWorkspaceForm({ navigationGuardRef, persistWeeklyService, persistAgendaRecords, date, setDate, teams, setTeams, activeTechs, customers, setCustomers, services, history, setHistory, updateTask, setNotice, weekly, setWeekly, databaseReady }) {
  ...
  const pendingDailyServices = teams.flatMap((team, teamIndex) => team.tasks.map((task, taskIndex) => ({ task, teamIndex, taskIndex })))
    .filter(({ task }) => !task.vehicleControl && taskHasContent(task) && !historyRecordForTask(task, date, history))
  ...
  const cancelDailyAndContinue = () => {
    ...
    if (first) {
      // Clear only the selected unfinished task, keeping its slot and every saved service.
      setTeams(previous => previous.map(team => ({ ...team, tasks: team.tasks.map(task =>
        task.taskId === first.task.taskId && !historyRecordForTask(task, date, history)
          ? { ...blankTask(), taskId: task.taskId, time: task.time }
          : task) })))
    }
    setDailyLeave(null)
