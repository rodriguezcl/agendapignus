...
function AgendaWorkspaceForm({ navigationGuardRef, persistWeeklyService, persistAgendaRecords, date, setDate, teams, setTeams, activeTechs, customers, setCustomers, services, history, setHistory, updateTask, setNotice, weekly, setWeekly, databaseReady }) {
  ...
  if (pastDayBlocked) {
    return <>{dailyLeave && <div className="modal-layer" style={{ zIndex: 10000 }}><section className="modal" role="alertdialog" aria-modal="true" aria-label="Servicios sin guardar"><h2>Servicios sin guardar</h2><p>Hay {pendingDailyServices.length} servicio(s) de esta agenda sin guardar. Guardá para continuar o volvé a completar la carga.</p><div className="modal-actions"><button type="button" className="secondary" onClick={() => setDailyLeave(null)}>Seguir editando</button><button type="button" className="primary" onClick={saveDailyAndContinue}>Guardar y continuar</button></div></section></div>}<div className="module-intro"><div><p className="eyebrow">PLANIFICACIÓN DIARIA</p><h1>Jornada finalizada</h1><p>Las fechas pasadas son de solo lectura en las agendas. Consultá o corregí los servicios registrados desde Historial.</p></div></div><div className="agenda-toolbar"><label><RequiredLabel>Fecha de trabajo</RequiredLabel><input required min={currentLocalDate()} type="date" value={date} onChange={event => setDate(event.target.value)} /></label><span>{prettyDate(date)}</span></div><p className="weekly-guard-advanced"><Icon name="lock" size={16} /><span>Esta jornada ya finalizó y no admite equipos ni servicios nuevos.</span></p></>
  }
  if (sundayBlocked) {
