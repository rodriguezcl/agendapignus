import { hasServiceActivity, protectServiceIdentity } from './domain/history/history-edit-policy.mjs'
...
function HistoryManagementDetail({ record, setHistory, close, customers, services, employees, authUser, persistHistoryRecord, persistHistoryRecordRemoval, historyScheduling }) {
  ...
  const identityLocked = hasServiceActivity(record)
  ...
  const saveChanges = () => {
    ...
    if (!identityLocked && (!customer || !service)) return
    const patch = { ...draft, ...(record.vehicleControl ? { estimatedMinutes: 15, estimatedMinutesCustomized: false } : {}), ...(!identityLocked ? { ...customerLinkPatch(record, customer, authUser), serviceId: service.id, service: service.name } : {}), teamId: record.teamId || '', team: record.team || '', technicianIds: record.technicianIds || [], technicians: record.technicians || [] }
    if (!persistHistoryRecord) window.dispatchEvent(new CustomEvent('pignus:sync-agenda-service', { detail: { record, patch: protectServiceIdentity(record, patch) } }))
    update(protectServiceIdentity(record, patch))
  }
