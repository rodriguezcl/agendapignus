...
function History({ history, setHistory, customers, services, employees, authUser, canManage = false, persistHistoryRecord, persistHistoryRecordRemoval, initialFilter = null }) {
  ...
  return <>{reminderFilter && <div className="pending-reminder" role="status"><Icon name="filter" /><div><b>{reminderFilter.pendingGroup === 'rescheduling' ? 'Reprogramación pendiente' : reminderFilter.pendingGroup === 'today' ? `Pendientes del ${prettyDate(reminderFilter.date)}` : 'Pendientes de días anteriores'}</b><span>{history.length} servicio(s) coincidente(s)</span></div><button className="secondary" onClick={() => setReminderFilter(null)}><Icon name="close" size={16} />Quitar filtro</button></div>}{canManage
    ? <HistoryManagement {...{ history, setHistory, customers, services, employees, authUser, persistHistoryRecord, persistHistoryRecordRemoval }} />
    : <HistoryReadOnly history={history} supervisorView={supervisorView} />}</>
}

function HistoryReadOnly({ history, supervisorView = false }) {
...
function HistoryBulkView({ history, setHistory, customers, services, employees, authUser, persistHistoryRecord, persistHistoryRecordRemoval }) {
  const [search, setSearch] = useState('')
  const [detail, setDetail] = useState(null)
...
function HistoryManagement({ history, setHistory, customers, services, employees, authUser, persistHistoryRecord, persistHistoryRecordRemoval }) {
  return <HistoryBulkView {...{ history, setHistory, customers, services, employees, authUser, persistHistoryRecord, persistHistoryRecordRemoval }} />
  const [search, setSearch] = useState('')
  const [detail, setDetail] = useState(null)
  const records = history.filter(record => normalizeSearchText(`${record.client} ${record.service} ${record.technicians?.join(' ')}`).includes(normalizeSearchText(search))).sort(sortHistoryByDateAndTime)
  const status = record => historyStatusLabel(record)
