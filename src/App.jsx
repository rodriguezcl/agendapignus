...
const INITIAL_SERVICES = [
  ...
  { id: 2, name: 'Instalación de cámaras', description: 'Instalación de videovigilancia', status: 'Activo' },
  { id: 3, name: 'Instalación de cerco eléctrico', description: 'Instalación de cerco perimetral', status: 'Activo' },
  ...
  useEffect(() => {
    ...
    const syncAgendaService = event => {
      ...
      setTeams(previous => previous.map((team, index) => index !== teamIndex ? team : {
        ...
        tasks: team.tasks.map(task => task.client === record.client && task.service === record.service ? { ...task, client: patch.client, service: patch.service, address: patch.address, phone: patch.phone, detail: patch.detail } : task)
      }))
    }
...
function AgendaWorkspaceForm({ date, setDate, teams, setTeams, activeTechs, customers, services, history, setHistory, updateTask, setNotice, weekly, setWeekly }) {
  ...
  const loadAgendaForDate = nextDate => {
    ...
    saved.forEach(record => {
      ...
      current.tasks.push({ taskId: record.sourceTaskId || record.id || createTaskId(), historyId: record.id, time: record.time || record.scheduledTime || record.hora || record.Hora || '', service: record.service || '', client: record.client || '', clientAccount: record.clientAccount || record.account || '', clientNameAtService: record.clientNameAtService || '', address: record.address || '', phone: record.phone || '', detail: record.detail || '', installationZone: record.installationZone || '' })
...
function DashboardView({ history }) {
  return <DashboardStatusView history={history} />
  const [month, setMonth] = useState(new Date().toISOString().slice(0, 7))
...
function HistoryView({ history, setHistory }) {
  return <HistoryManagement history={history} setHistory={setHistory} />
  const [search, setSearch] = useState('')
...
function HistoryBulkView({ history, setHistory }) {
  const [search, setSearch] = useState('')
  const [detail, setDetail] = useState(null)
...
function HistoryManagement({ history, setHistory }) {
  return <HistoryBulkView history={history} setHistory={setHistory} />
  const [search, setSearch] = useState('')
  const [detail, setDetail] = useState(null)
  const records = history.filter(record => `${record.client} ${record.service} ${record.technicians?.join(' ')}`.toLowerCase().includes(search.toLowerCase())).sort((a, b) => b.date.localeCompare(a.date))
  const status = record => record.status || 'Pendiente'
...
function Accounts({ customers, setCustomers, setNotice, ask }) {
  ...
  const save = e => { e.preventDefault(); const customer = { ...form, account: normalizeAccountKey(form.account), address: [form.street, form.locality, form.province].filter(Boolean).join(', ') }; ask(editing ? 'Confirmar edición' : 'Confirmar alta', `¿Querés guardar los cambios de ${customer.name}?`, () => { setCustomers(prev => editing ? prev.map(c => normalizeAccountKey(c.account) === normalizeAccountKey(editing) ? customer : c) : [...prev.filter(c => normalizeAccountKey(c.account) !== customer.account), customer]); setShowForm(false); setEditing(null); setNotice('Los datos del cliente fueron guardados.') }) }
...
function ServiceTypes({ services, setServices, setNotice, ask }) {
  const [form, setForm] = useState({ name: '', description: '', status: 'Activo' })
  const [editing, setEditing] = useState(null)
  const [open, setOpen] = useState(false)
  const save = event => {
    event.preventDefault()
    const record = { ...form, id: editing || Date.now() }
...
function ImportModal({ customers, setCustomers, close, setNotice }) {
  ...
  const importFile = async e => {
    ...
    const imported = rows.map(row => {
      ...
      return account ? { account, name: get(row, 'nombre'), type: get(row, 'tipodecuenta'), street, locality, province, phone: get(row, 'telefono'), address: [street, locality, province].filter(Boolean).join(', '), fields: Object.fromEntries(headers.map((h, i) => [h, row[i] || ''])) } : null
    }).filter(Boolean)
