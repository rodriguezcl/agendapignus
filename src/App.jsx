...
function CustomerAutocomplete({ value = '', customerId = '', customers = [], subscriberReservation = false, awaitingConfirmation = false, onTextCommit, onCustomerSelect, onAddCustomer, onReserveSubscriber, className = '' }) {
...
function AgendaWorkspaceForm({ navigationGuardRef, persistWeeklyService, persistAgendaRecords, date, setDate, teams, setTeams, activeTechs, customers, setCustomers, services, history, setHistory, updateTask, setNotice, weekly, setWeekly, databaseReady }) {
  ...
  const dailyCustomerField = (task, teamIndex, taskIndex) => isTrainingService(serviceForTask(task)) ? <label className="daily-field-customer"><RequiredLabel>Nombre de la capacitación</RequiredLabel><BufferedInput aria-required="true" placeholder="CAPACITACIÓN CERCO ELÉCTRICO" value={task.client} onCommit={value => updateTask(teamIndex, taskIndex, trainingClientPatch(value))} /></label> : <DailyCustomerField task={task} customers={customers} teamIndex={teamIndex} taskIndex={taskIndex} onTextCommit={commitCustomerText} onCustomerSelect={selectCustomerResult} onAddCustomer={beginNewCustomer} onReserveSubscriber={serviceCode(serviceForTask(task)) === 'alarm-installation' ? beginSubscriberReservation : undefined} />
