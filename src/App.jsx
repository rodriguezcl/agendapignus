...
function History({ history, setHistory, customers, services, employees, authUser, canManage = false, persistHistoryRecord }) {
  return canManage
    ? <HistoryManagement {...{ history, setHistory, customers, services, employees, authUser, persistHistoryRecord }} />
...
function HistoryBulkView({ history, setHistory, customers, services, employees, authUser, persistHistoryRecord }) {
  const [search, setSearch] = useState('')
  const [detail, setDetail] = useState(null)
...
function HistoryManagementDetail({ record, setHistory, close, customers, services, employees, authUser, persistHistoryRecord }) {
  ...
  const remove = () => { if (interactionBlocked) return; setHistory(previous => previous.filter(item => item.id !== record.id)); close() }
  const saveChanges = () => {
    if (interactionBlocked) return
