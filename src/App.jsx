...
function EmployeeForm({ form, setForm, roles, save, cancel, editing, canEnableTechnical = false }) { const set = key => e => setForm({ ...form, [key]: e.target.value }); return <EditorModal active={Boolean(editing)} title="Editar empleado" onClose={cancel}><form className="employee-form employee-form-wide" onSubmit={save}><label><RequiredLabel>Nombre</RequiredLabel><input required value={form.firstName || ''} onChange={set('firstName')} /></label><label><RequiredLabel>Apellido</RequiredLabel><input required value={form.lastName || ''} onChange={set('lastName')} /></label><label><RequiredLabel>Rol</RequiredLabel><select required value={form.roleId ?? roles.find(role => role.name === form.role)?.id ?? ''} onChange={event => { const selectedRole = roles.find(role => String(role.id) === event.target.value); setForm({ ...form, roleId: selectedRole?.id, role: selectedRole?.name || '' }) }}>{roles.map(r => <option key={r.id} value={r.id}>{r.name}</option>)}</select></label><label><input type="checkbox" checked={form.technicalEnabled === true} disabled={!canEnableTechnical} onChange={event => setForm({ ...form, technicalEnabled: event.target.checked })} />Puede realizar servicios técnicos<small>Conserva su rol y agrega Mis servicios. Solo Administración puede cambiar esta habilitación.</small></label><label>Teléfono<input value={form.phone} onChange={set('phone')} /></label><label><RequiredLabel>Correo electrónico</RequiredLabel><input required type="email" value={form.email} onChange={set('email')} /></label><label>{editing ? 'Contraseña' : <RequiredLabel>Contraseña</RequiredLabel>}<input required={!editing} minLength="8" autoComplete="new-password" type="password" value={form.password || ''} placeholder={editing ? 'Dejar vacío para conservarla' : 'Mínimo 8 caracteres'} onChange={set('password')} /></label><div className="employee-form-actions"><button className="primary"><Icon name="check" />{editing ? 'Guardar cambios' : 'Guardar empleado'}</button><button type="button" className="secondary" onClick={cancel}><Icon name="close" size={16} />Cancelar</button></div></form></EditorModal> }
