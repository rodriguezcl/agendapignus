...
function HistoryManagementDetail({ record, setHistory, close, customers, services, employees, authUser, persistHistoryRecord, persistHistoryRecordRemoval, historyScheduling }) {
  ...
  const saveChanges = () => {
    ...
    const patch = { ...draft, ...(record.vehicleControl ? { estimatedMinutes: 15, estimatedMinutesCustomized: false } : {}), ...(customer ? customerLinkPatch(record, customer, authUser) : {}), ...(service ? { serviceId: service.id, service: service.name } : {}), address: draft.address, phone: draft.phone, teamId: record.teamId || '', team: record.team || '', technicianIds: record.technicianIds || [], technicians: record.technicians || [] }
