...
import { historyRescheduleOperations } from './features/state/application/history-reschedule.mjs'
import { availableRescheduleTeams, completeRescheduleSlot } from './domain/agenda/reschedule-availability.mjs'
...
import { serviceGaps, planningHoursForDay } from './domain/agenda/service-gaps.mjs'
...
import { HelpShell, TechnicianHelpCenter } from './HelpCenter.jsx'
...
import { countYearToDateAlarmInstallations, countYearToDateCompletedRecords, dashboardPendingGroups, historyReminderRecords, historyStatusLabel } from './domain/dashboard/dashboard-metrics.mjs'
...
import { serviceCode, isTrainingService, trainingClientPatch } from './domain/services/service.mjs'
...
import { weeklyServiceOperations, weeklyServiceMoveOperations, vehicleControlSwapCandidate } from './features/state/application/weekly-service-save.mjs'
...
const servicePhotoUrl = record => record?.servicePhotoUrl || (record?.servicePhotoAttached ? `/api/service-photo/${encodeURIComponent(String(record.id || record.historyId || ''))}` : '')

function ServicePhoto({ record, className = '' }) {
  const url = servicePhotoUrl(record)
  const [visible, setVisible] = useState(Boolean(url))
  useEffect(() => setVisible(Boolean(url)), [url])
  if (!url || !visible) return null
  return <a className={`service-photo ${className}`.trim()} href={url} target="_blank" rel="noreferrer" title="Abrir foto del servicio"><img src={url} alt={`Foto de referencia de ${record.client || record.service || 'servicio'}`} onError={() => setVisible(false)} /></a>
}

function ServicePhotoField({ record, photoData = '', removed = false, onChange, disabled = false }) {
  const galleryRef = useRef(null)
  const cameraRef = useRef(null)
  const [processing, setProcessing] = useState(false)
  const [error, setError] = useState('')
  const existingUrl = servicePhotoUrl(record)
  const preview = removed ? '' : photoData || existingUrl
  const select = async file => {
    if (!file || disabled) return
    setProcessing(true); setError('')
    try { onChange({ photoData: await compactVehiclePhoto(file), removed: false }) }
    catch (uploadError) { setError(uploadError.message) }
    finally { setProcessing(false) }
  }
  return <section className="service-photo-field"><div><b>Foto de referencia</b><small>Visible para el equipo técnico asignado.</small></div>{preview && <img src={preview} alt="Vista previa de la foto del servicio" onError={event => { if (!photoData) event.currentTarget.hidden = true }} />}<div className="service-photo-actions"><button type="button" className="secondary small" disabled={disabled || processing} onClick={() => galleryRef.current?.click()}><Icon name="upload" size={16} />Galería o archivo</button><button type="button" className="secondary small" disabled={disabled || processing} onClick={() => cameraRef.current?.click()}><Icon name="camera" size={16} />Sacar foto</button>{preview && <button type="button" className="danger-button small" disabled={disabled || processing} onClick={() => onChange({ photoData: '', removed: true })}><Icon name="trash" size={16} />Quitar foto</button>}</div><input ref={galleryRef} hidden type="file" accept="image/*" onChange={event => { select(event.target.files?.[0]); event.target.value = '' }} /><input ref={cameraRef} hidden type="file" accept="image/*" capture="environment" onChange={event => { select(event.target.files?.[0]); event.target.value = '' }} />{processing && <small>Preparando foto…</small>}{error && <small className="field-error" role="alert">{error}</small>}</section>
}

function ServicePhotoManager({ record, onMetadataChange }) {
  const [saving, setSaving] = useState(false)
  const [error, setError] = useState('')
  const recordId = record?.id || record?.historyId
  const change = async ({ photoData, removed }) => {
    if (!recordId || saving) return
    setSaving(true); setError('')
    try {
      if (photoData) {
        const result = await servicePhotoRepository.upload(recordId, photoData)
        onMetadataChange({ servicePhotoAttached: true, servicePhotoUrl: result.url, servicePhotoAttachedAt: result.createdAt })
      } else if (removed) {
        await servicePhotoRepository.remove(recordId)
        onMetadataChange({ servicePhotoAttached: false, servicePhotoUrl: '', servicePhotoAttachedAt: '' })
      }
    } catch (photoError) { setError(photoError.message) }
    finally { setSaving(false) }
  }
  return <div className="service-photo-manager"><ServicePhotoField record={record} disabled={!recordId || saving} onChange={change} />{!recordId && <small>Guardá el servicio antes de adjuntar una foto.</small>}{error && <small className="field-error" role="alert">{error}</small>}</div>
}
...
function recordHolidayDecision(setWeekly, setNotice, date, holiday, status, { weekly = {}, history = [], setHistory, holidays = [] } = {}) {
  ...
  const sourceHistory = history || globalThis.__pignusHistory || []
  const rescheduledHistory = rescheduleVehicleControlRecords(sourceHistory, { holidays, holidayOverrides: nextWeekly._holidayOverrides })
  const movedControls = sourceHistory.map((record, index) => ({ before: record, after: rescheduledHistory[index] })).filter(({ before, after }) => before?.vehicleControl && before.date !== after.date)
  movedControls.forEach(({ before, after }) => { nextWeekly = moveRecordInWeeklyAgenda(nextWeekly, after, after.date, before.date) })
  setWeekly(nextWeekly)
  const replaceHistory = setHistory || globalThis.__pignusSetHistory
  if (replaceHistory) replaceHistory(previous => rescheduleVehicleControlRecords(previous, { holidays, holidayOverrides: nextWeekly._holidayOverrides }))
  const controlNotice = movedControls.length ? ` Se reprogramaron ${movedControls.length} control(es) vehicular(es) pendiente(s) al último día operativo anterior.` : ''
  setNotice(`${status === 'working' ? `${prettyDate(date)} quedó habilitado como día laboral.` : `${prettyDate(date)} quedó definido como día no operativo.`}${controlNotice}`)
}
...
const INSTALLATION_ZONES = [
  ...
  ['nobu-town', 'Nobu Town'],
  ['residencial', 'Residencial con monitoreo'],
  ['no-monitoreada', 'Residencial sin monitoreo']
]
...
const createInternalChecklistItem = () => ({ id: createTaskId(), text: '', completed: false })
const normalizeInternalChecklist = value => Array.isArray(value) ? value.map((item, index) => ({
  id: String(item?.id || `checklist-${index}`),
  text: String(item?.text || ''),
  completed: Boolean(item?.completed)
})) : []
const blankTask = () => ({ taskId: createTaskId(), time: '', serviceId: '', service: '', customerId: '', client: '', clientAccount: '', clientNameAtService: '', address: '', phone: '', detail: '', internalNote: '', internalChecklist: [], paymentMethod: '', amount: '', monthlyFee: '', form: '', formEmail: '' })
...
const taskHasContent = task => Boolean(task && (
  ...
  normalizeInternalChecklist(task.internalChecklist).some(item => item.text.trim())
))
...
const conflictTaskIndex = (task, index) => Number.isInteger(Number(task?._scheduleTaskIndex)) ? Number(task._scheduleTaskIndex) : index
const conflictTaskName = (task, index) => {
...
const conflictIntervalDescription = (task, interval) => interval.earlyCompletion
...
const scheduleConflictForTaskMessage = (conflict, taskIndex) => {
  ...
  const secondIndex = conflictTaskIndex(conflict.secondTask, conflict.secondTaskIndex)
  const currentIsFirst = firstIndex === taskIndex
  const currentIsSecond = secondIndex === taskIndex
  if (!currentIsFirst && !currentIsSecond) return `No se puede agendar en este horario porque ${scheduleConflictMessage(conflict)}.`
  const currentTask = currentIsFirst ? conflict.firstTask : conflict.secondTask
  const currentInterval = currentIsFirst ? conflict.first : conflict.second
  const currentFallbackIndex = currentIsFirst ? conflict.firstTaskIndex : conflict.secondTaskIndex
  const otherTask = currentIsFirst ? conflict.secondTask : conflict.firstTask
  const otherInterval = currentIsFirst ? conflict.second : conflict.first
  const otherFallbackIndex = currentIsFirst ? conflict.secondTaskIndex : conflict.firstTaskIndex
  const currentName = conflictTaskName(currentTask, currentFallbackIndex)
  const otherName = conflictTaskName(otherTask, otherFallbackIndex)
  const currentOccupation = conflictIntervalDescription(currentTask, currentInterval).replace(/^ocupa/, 'ocuparía').replace(' y reserva ', ' y reservaría ')
  return `El ${currentName} no puede comenzar a las ${currentInterval.startTime} porque se superpone con el ${otherName}, que ${conflictIntervalDescription(otherTask, otherInterval)}. El ${currentName} ${currentOccupation}.`
}
...
const serviceTrace = record => ({
  ...(record?.serviceJourney ? { serviceJourney: record.serviceJourney } : {}),
...
const blankCustomer = { customerId: '', kind: 'client', account: '', name: '', type: '', street: '', locality: '', province: '', phone: '', address: '', cctvService: false, fields: {} }
...
function CustomerAutocomplete({ value = '', customerId = '', customers = [], subscriberReservation = false, awaitingConfirmation = false, onTextCommit, onCustomerSelect, onAddCustomer, onReserveSubscriber, className = '', required = true }) {
  ...
  const commitText = () => {
    ...
    else if (query !== value) { if (onTextCommit) onTextCommit(query); else setQuery(value || '') }
  }
  ...
  return <div className={`customer-autocomplete ${className}`.trim()}>
    <label><RequiredLabel required={required}>Abonado o cliente</RequiredLabel></label>
    <div className="customer-autocomplete-control">
      <input autoComplete="off" spellCheck={false} role="combobox" aria-required={required} aria-autocomplete="list" aria-expanded={showResults} placeholder="Buscá por nombre, cuenta o teléfono" value={query} onFocus={() => setOpen(true)} onChange={event => { setQuery(event.target.value); setOpen(true) }} onKeyDown={handleKeyDown} onBlur={() => { commitText(); setOpen(false) }} />
      <span aria-hidden="true">⌄</span>
    </div>
    {awaitingConfirmation && <span className="service-confirmation-label">(A CONFIRMAR)</span>}
...
const serviceEstimateForTask = (task, service) => task?.estimatedMinutesCustomized === false
  ...
  : normalizeServiceEstimatedMinutes(task?.estimatedMinutes, normalizeServiceEstimatedMinutes(service?.estimatedMinutes))
...
function ServiceEstimatedDurationField({ value, onChange, className = '', required = true }) {
  ...
  return <div className={`task-duration-field ${className}`.trim()}><span><RequiredLabel required={required}>Tiempo estimado</RequiredLabel></span><div><label><BufferedInput aria-label="Horas estimadas del servicio" type="number" inputMode="numeric" min="0" max="12" step="1" value={hours} onCommit={value => update(value, minutes)} /><small>h</small></label><label><select aria-label="Minutos estimados del servicio" value={minutes} onChange={event => update(hours, event.target.value)}><option value="0">00</option><option value="15">15</option><option value="30">30</option><option value="45">45</option></select><small>min</small></label></div></div>
}
...
const technicianVisibleServiceExtras = task => {
  const paymentMethod = String(task?.paymentMethod || '').trim()
  const form = String(task?.form || '').trim()
  const cashPayment = normalizeServiceName(paymentMethod) === 'efectivo'
  const handwrittenForm = normalizeServiceName(form).startsWith('incompleto')
  return {
    paymentMethod: cashPayment ? 'Efectivo' : '',
    amount: cashPayment ? String(task?.amount || '').trim() : '',
    form: handwrittenForm ? form : ''
  }
}
...
const normalizeFormValue = value => {
  ...
  if (normalized === 'completo' || normalized === 'completado') return FORM_OPTIONS[0]
...
const applicableServiceExtras = (task, service) => {
  ...
  return {
    ...
    formEmail: available.form && normalizeFormValue(task?.form) === FORM_OPTIONS[0] ? String(task?.formEmail || '').trim() : ''
  }
}
const invalidFormEmail = (task, service) => {
  const email = applicableServiceExtras(task, service).formEmail
...
const weeklyTaskMissingFields = (task, service) => {
  if (task?.vehicleControl) {
    return [['hora', task?.time], ['tipo de servicio', task?.serviceId || task?.service], ['vehículo', task?.vehicleId], ['técnico responsable', task?.technicianIds?.[0]]]
      .filter(([, value]) => !String(value || '').trim())
      .map(([label]) => label)
  }
  const missing = [['hora', task?.time], ['tipo de servicio', task?.service], ['cliente', task?.customerId || ((task?.newCustomer || task?.subscriberReservation || isTrainingService(service)) && task?.client)], ['dirección', task?.address], ['contacto', isTrainingService(service) || task?.phone], ['detalle', task?.detail]]
    .filter(([, value]) => !String(value || '').trim())
    .map(([label]) => label)
  ...
  if (invalidFormEmail(task, service)) missing.push('email del formulario válido')
  return missing
}
...
function technicianTimeConflicts(teams, employees = []) {
  ...
  teams.forEach((team, teamIndex) => {
    ...
    ;(team.tasks || []).filter(task => task.time && [task.service, task.customerId, task.client].some(value => String(value || '').trim())).forEach(task => {
      const taskEmployeeIds = task.vehicleControl && task.technicianIds?.length ? task.technicianIds : employeeIds
      const taskEmployeeNames = task.vehicleControl && task.technicians?.length ? task.technicians : employeeNames
      const technicians = taskEmployeeIds.length
        ? taskEmployeeIds.map(id => ({ key: String(id), name: employees.find(employee => String(employee.id) === String(id))?.name || taskEmployeeNames[taskEmployeeIds.findIndex(value => String(value) === String(id))] || 'Técnico' }))
        : taskEmployeeNames.map(name => ({ key: `name:${normalizeSearchText(name)}`, name }))
...
export default function App() {
  ...
  const activeTechs = employees.filter(employee => employee.status === 'Activo' && (roleCode(employeeRole(employee)) === 'technician' || (roleCode(employeeRole(employee)) !== 'supervisor' && employee.technicalEnabled === true)))
  ...
  useEffect(() => {
    ...
    const previousDefaults = serviceDefaultsRef.current
    const nextDefaults = new Map(services.map(service => [String(service.id), normalizeServiceEstimatedMinutes(service.estimatedMinutes)]))
    ...
    serviceDefaultsRef.current = nextDefaults
  }, [services])
  useEffect(() => {
  ...
  useEffect(() => {
    fetchAuthWithRetry('/api/auth/session', { cache: 'no-store', credentials: 'same-origin' }).then(async response => {
      const data = await response.json().catch(() => ({}))
      if (!response.ok) {
        if (data.code === 'SESSION_ENDED') setSessionEndedMessage(data.error)
        return null
      }
      return data
    }).then(data => {
  ...
  useEffect(() => {
    const goToHistory = event => { setHistoryEntryFilter(event.detail?.pendingGroup ? event.detail : null); setModule('history') }
    ...
    const syncAgendaService = event => {
      ...
      const updateTeamsForRecord = previous => previous.map((team, index) => (record.teamId ? String(team.teamId) !== String(record.teamId) : index !== legacyTeamIndex) ? team : {
        ...
        tasks: team.tasks.map(task => {
          ...
          return sameTask ? { ...task, customerId: patch.customerId ?? task.customerId, clientAccount: patch.clientAccount ?? task.clientAccount, clientNameAtService: patch.clientNameAtService ?? task.clientNameAtService, client: patch.client ?? task.client, serviceId: patch.serviceId ?? task.serviceId, service: patch.service ?? task.service, address: patch.address ?? task.address, phone: patch.phone ?? task.phone, detail: patch.detail ?? task.detail, internalNote: patch.internalNote ?? task.internalNote, internalChecklist: patch.internalChecklist ?? task.internalChecklist, ...serviceTrace(patch) } : task
        })
      })
    ...
    const syncWeeklyTask = event => {
      ...
      setHistory(previous => {
        ...
        if (previous.some(sameRecord)) return previous.map(record => sameRecord(record) ? {
          ...
          estimatedMinutesCustomized: Boolean(draft.estimatedMinutesCustomized),
          ...
          address: draft.address,
           ...
           internalChecklist: normalizeInternalChecklist(draft.internalChecklist),
           paymentMethod: draft.paymentMethod || '',
           amount: draft.amount || '',
           monthlyFee: draft.monthlyFee || '',
           awaitingConfirmation: draft.awaitingConfirmation === true, form: draft.form || '', formEmail: draft.formEmail || '',
  ...
  const persistWeeklyService = command => persistStateCommand(snapshot => (
    command.operation === 'service-journeys'
      ? planServiceJourneyOperations(snapshot, { ...command, today: currentLocalDate(), teamsForDate: reschedulingTeams, createId: createTaskId })
      : command.operation === 'service-confirmation'
      ? serviceConfirmationOperations(snapshot, { ...command, today: currentLocalDate() })
  ...
  const reschedulingTeams = (day, sourceWeekly = weekly) => {
    if (!day || isSunday(day)) return []
    const plan = dailyPlanWithMonthlyTeams(day, sourceWeekly)
    let choices = renumberVisibleWeeklyTeams(applyRemovedWeeklyTeams(applyRemovedWeeklySlots(applyRemovedWeeklyTasks(plan?.teams || [], plan?.removedTaskIds || []), plan?.removedSlots || []), plan?.removedTeams || []))
    if (isSaturday(day)) {
      if (!choices.length && !plan?.removedTeams?.length) choices = [{ teamId: `history-guard-${day}`, label: 'Equipo 1', memberIds: [], members: [], tasks: [] }]
      choices = assignGuardToEmptySaturday(normalizeSaturdayTeams(choices, day, sourceWeekly), day, sourceWeekly, activeTechs)
      if (advancedGuardForSaturdayDate(day, sourceWeekly, choices)) return []
    }
    return choices.filter(team => team.memberIds?.length)
  }
  const historyScheduling = {
    assignedTeamsForDate: day => reschedulingTeams(day),
    teamsForDate: (day, time, record) => {
      if (!completeRescheduleSlot(day, time)) return []
      const choices = reschedulingTeams(day)
      const occupied = choices.map(team => ({ ...team, tasks: (team.tasks || []).map(task => taskForScheduleOccupancy(taskWithServiceEstimate(task, resolveServiceForTask(task, services)), day, history)).filter(Boolean) }))
      return availableRescheduleTeams(occupied, taskWithServiceEstimate(record, resolveServiceForTask(record, services)), day, time)
    },
    save: (base, day, teamId, time) => persistStateCommand(snapshot => {
      const team = reschedulingTeams(day, snapshot.agenda?.weekly).find(item => String(item.teamId) === String(teamId))
      return historyRescheduleOperations(snapshot, { base, day, time, team, today: currentLocalDate() })
  ...
  const persistHistoryRecordRemoval = async record => {
    try {
      ...
      setNotice(`No se eliminó el servicio. ${error.message || 'Intentá nuevamente.'}`)
      throw error
    }
  }
  const applyCustomerImportState = payload => {
    hydratingStateRef.current = true
  ...
  useEffect(() => {
    ...
    const refreshWhenVisible = () => {
      if (document.visibilityState !== 'hidden') void refreshRemoteState()
    }
  ...
  const technicalEnabled = !isSupervisor && (employees.find(employee => String(employee.id) === String(authUser?.id))?.technicalEnabled ?? authUser?.technicalEnabled) === true
  modulePermissions.myServices = technicalEnabled
  if (technicalEnabled) nav.push(['myServices', 'tools', 'Mis servicios'])
  if (!isAdministrator) {
  ...
  }, [isAdministrator, module, activeRole?.id, technicalEnabled])
  ...
  if (module === 'myServices' && technicalEnabled) return <PersonalTechnicalServices user={authUser} logout={logout} sessionInvalidated={endInvalidatedSession} onBack={() => setModule('dashboard')} />
  ...
  return <ServiceJourneysContext.Provider value={{history, today: currentLocalDate(), enabled: !isSupervisor && (isAdministrator || Boolean(modulePermissions.agenda || modulePermissions.weekly)), teamsForDate: reschedulingTeams, availableTeams: historyScheduling.teamsForDate, save: async (base, visits) => {
        for (const year of new Set(visits.map(visit => String(visit.date).slice(0,4)))) {
          const response = await fetchWithTimeout('/api/holidays?year=' + encodeURIComponent(year), { credentials: 'same-origin' })
          if (!response.ok) throw new Error('No se pudo verificar el calendario laboral. No se guardaron las jornadas.')
...
function Agenda({ navigationGuardRef, persistWeeklyService, persistAgendaRecords, date, setDate, teams, setTeams, activeTechs, customers, setCustomers, services, history, setHistory, updateTask, setNotice, weekly, setWeekly, databaseReady, authUser }) {
  ...
  return <AgendaLayout navigationGuardRef={navigationGuardRef} {...{ persistWeeklyService, persistAgendaRecords, date, setDate, teams, setTeams, activeTechs, customers, setCustomers, services, history, setHistory, updateTask, setNotice, weekly, setWeekly, databaseReady }} />
...
function AgendaLayout({ navigationGuardRef, persistWeeklyService, persistAgendaRecords, date, setDate, teams, setTeams, activeTechs, customers, setCustomers, services, history, setHistory, updateTask, setNotice, weekly, setWeekly, databaseReady }) {
  return <AgendaWorkspace navigationGuardRef={navigationGuardRef} {...{ persistWeeklyService, persistAgendaRecords, date, setDate, teams, setTeams, activeTechs, customers, setCustomers, services, history, setHistory, updateTask, setNotice, weekly, setWeekly, databaseReady }} />
...
function historicalAgendaTeams(date, weekly, history) {
  const day = weekly?.[date] || {}
  const removed = new Set((day.removedTaskIds || []).map(String))
  const result = applyRemovedWeeklyTeams(applyRemovedWeeklySlots(applyRemovedWeeklyTasks(day.teams || [], day.removedTaskIds || []), day.removedSlots || []), day.removedTeams || [])
    .map(team => ({ ...team, tasks: (team.tasks || []).map(task => ({ ...task, ...historyRecordForTask(task, date, history) })) }))
  for (const record of history.filter(item => item.date === date)) {
    if (removed.has(String(record.sourceTaskId || '')) || removed.has(`task:${record.sourceTaskId}`) || removed.has(`history:${record.id}`)) continue
    if (result.some(team => team.tasks.some(task => String(task.historyId || task.id || '') === String(record.id) || (record.sourceTaskId && task.taskId === record.sourceTaskId)))) continue
    let team = result.find(item => record.teamId ? item.teamId === record.teamId : item.label === record.team)
    if (!team) {
      team = { teamId: record.teamId, label: record.team || 'Equipo', members: record.technicians || [], tasks: [] }
      result.push(team)
    }
    team.tasks.push({ ...record, historyId: record.id, taskId: record.sourceTaskId })
  }
  return result.map(team => ({ ...team, tasks: sortTasksByTime(team.tasks) }))
}

function HistoricalDailyAgenda({ date, setDate, weekly, history }) {
  const historicalTeams = historicalAgendaTeams(date, weekly, history)
  return <><div className="module-intro"><div><p className="eyebrow">PLANIFICACIÓN DIARIA · SOLO LECTURA</p><h1>Agenda del día</h1><p>Jornada finalizada. Podés consultar la información guardada, sin modificarla.</p></div></div>
    <div className="agenda-toolbar"><label>Fecha de trabajo<input type="date" value={date} onChange={event => setDate(event.target.value)} /></label><span>{prettyDate(date)}</span></div>
    {!historicalTeams.length && <p>No hay una agenda guardada para esta fecha.</p>}
    {historicalTeams.map((team, index) => <article className="team-card" key={team.teamId || index}><div className="team-header"><strong>{team.label || `Equipo ${index + 1}`}</strong><span>{team.members?.join(' · ') || 'Sin técnicos asignados'}</span></div><div className="historical-day-services">{team.tasks.map((task, taskIndex) => {
      const pastService = { ...task, team: team.label || `Equipo ${index + 1}`, technicians: task.technicians || team.members }
      return <section className="historical-day-service" key={task.taskId || task.id || taskIndex}><h2>Servicio {taskIndex + 1} · {task.time} hs</h2>{!taskHasContent(task) ? <p>Turno sin servicio asignado</p> : <><h3>{task.client || task.service}</h3><HistoricalServiceFields pastService={pastService} /></>}</section>
    })}</div></article>)}</>
}

function HistoricalServiceFields({ pastService }) {
  return <>{[
      ...
      ['Ubicación y contacto', [
        ['Dirección', pastService.address], ['Contacto', pastService.phone], ['Ubicación de la instalación', pastService.installationZone]
      ]],
      ['Pago y documentación', [
        ['Forma de pago', pastService.paymentMethod], ['Monto', formatCurrencyAmount(pastService.amount)], ['Abono mensual', formatCurrencyAmount(pastService.monthlyFee)], ['Formulario', pastService.form], ['Email del formulario', pastService.formEmail]
      ]],
      ['Observaciones e información interna', [
        ['Detalle / observaciones', pastService.detail], ['Informe técnico', pastService.technicalObservation], ['Nota interna', pastService.internalNote]
      ]]
    ].map(([title, fields]) => <section className="past-detail-section" key={title}><h3>{title}</h3><dl className="past-detail-fields">{fields.map(([label, value]) => <div key={label} className={['Detalle / observaciones', 'Informe técnico', 'Nota interna'].includes(label) ? 'past-field-wide' : ['Dirección', 'Técnicos asignados'].includes(label) ? 'past-field-double' : ''}><dt>{label}</dt><dd className={['Monto', 'Abono mensual'].includes(label) ? 'past-field-money' : ''}>{value === '' || value == null ? <span className="past-field-empty">Sin información</span> : String(value)}</dd></div>)}</dl></section>)}{pastService.internalChecklist?.length > 0 && <section className="past-detail-section"><h3>Checklist</h3>{pastService.internalChecklist.map((item, index) => <p key={item.id || index}>{item.completed ? '✓' : '○'} {item.text}</p>)}</section>}<ServicePhoto record={pastService} className="history-service-photo" /></>
}

function AgendaWorkspace({ navigationGuardRef, persistWeeklyService, persistAgendaRecords, date, setDate, teams, setTeams, activeTechs, customers, setCustomers, services, history, setHistory, updateTask, setNotice, weekly, setWeekly, databaseReady }) {
  if (String(date || '') < currentLocalDate()) return <HistoricalDailyAgenda {...{ date, setDate, weekly, history }} />
  return <AgendaWorkspaceForm navigationGuardRef={navigationGuardRef} {...{ persistWeeklyService, persistAgendaRecords, date, setDate, teams, setTeams, activeTechs, customers, setCustomers, services, history, setHistory, updateTask, setNotice, weekly, setWeekly, databaseReady }} />
  const [preview, setPreview] = useState(false)
  const [techOpen, setTechOpen] = useState(null)
  const [filter, setFilter] = useState('')
  const [confirmation, setConfirmation] = useState(null)
...
function DailyCustomerField({ task, customers, teamIndex, taskIndex, onTextCommit, onCustomerSelect, onAddCustomer, onReserveSubscriber, required = true }) {
  return <CustomerAutocomplete
    awaitingConfirmation={task.awaitingConfirmation === true}
...
function InternalPreparationFields({ task, onChange, buffered = false, readOnly = false }) {
  const checklist = normalizeInternalChecklist(task?.internalChecklist)
  const changeChecklist = next => onChange({ internalChecklist: normalizeInternalChecklist(next) })
  const updateItem = (id, patch) => changeChecklist(checklist.map(item => item.id === id ? { ...item, ...patch } : item))
  const note = String(task?.internalNote || '')
  if (readOnly) return <section className="internal-preparation internal-preparation-readonly"><div className="internal-preparation-heading"><div><b>Nota interna</b><small>Información operativa para Administración y usuarios</small></div></div><p className="internal-note-value">{note.trim() || 'Sin nota interna'}</p>{checklist.length > 0 && <div className="internal-checklist"><b>Checklist de preparación</b>{checklist.map(item => <label className="internal-checklist-item readonly" key={item.id}><input type="checkbox" checked={item.completed} readOnly /><span className={item.completed ? 'completed' : ''}>{item.text || 'Elemento sin descripción'}</span></label>)}</div>}</section>
  return <section className="internal-preparation"><div className="internal-preparation-heading"><div><b>Nota interna</b><small>Visible sólo para Administración y usuarios. No se comparte con técnicos.</small></div>{checklist.length === 0 && <button type="button" className="secondary compact" onClick={() => changeChecklist([createInternalChecklistItem()])}><Icon name="check" size={15} />Crear checklist</button>}</div><label className="internal-note-field">Anotaciones{buffered ? <BufferedTextarea maxLength={5000} value={note} onCommit={value => onChange({ internalNote: value })} placeholder="Ej.: No pedir al proveedor; hay stock disponible." /> : <textarea maxLength={5000} value={note} onChange={event => onChange({ internalNote: event.target.value })} placeholder="Ej.: No pedir al proveedor; hay stock disponible." />}</label>{checklist.length > 0 && <div className="internal-checklist"><div className="internal-checklist-heading"><b>Checklist de preparación</b><button type="button" className="secondary compact" disabled={checklist.length >= 30} onClick={() => changeChecklist([...checklist, createInternalChecklistItem()])}><Icon name="plus" size={14} />Agregar elemento</button></div>{checklist.map(item => <div className="internal-checklist-item" key={item.id}><input type="checkbox" checked={item.completed} aria-label={`Marcar ${item.text || 'elemento'} como preparado`} onChange={event => updateItem(item.id, { completed: event.target.checked })} /><BufferedInput maxLength={500} value={item.text} onCommit={value => updateItem(item.id, { text: value })} placeholder="Elemento a preparar" /><button type="button" className="icon-btn delete" title="Eliminar elemento" aria-label="Eliminar elemento del checklist" onClick={() => changeChecklist(checklist.filter(current => current.id !== item.id))}><Icon name="trash" size={15} /></button></div>)}</div>}</section>
}
...
function ServiceExtraFields({ className, task, service, onChange, buffered = false }) {
  ...
  return <><InternalPreparationFields task={task} onChange={onChange} buffered={buffered} /><div className={className}>{fields.map(([key, label]) => {
...
function AgendaWorkspaceForm({ navigationGuardRef, persistWeeklyService, persistAgendaRecords, date, setDate, teams, setTeams, activeTechs, customers, setCustomers, services, history, setHistory, updateTask, setNotice, weekly, setWeekly, databaseReady }) {
  ...
  const singleSaveRef = useRef(false)
  const [singleSaving, setSingleSaving] = useState(false)
  const [dailyLeave, setDailyLeave] = useState(null)
  const [dailyResume, setDailyResume] = useState(null)
  const [dailyPlanningClock, setDailyPlanningClock] = useState(Date.now)
  const [gapTaskFocus, setGapTaskFocus] = useState(null)
  useEffect(() => {
    const timer = window.setInterval(() => setDailyPlanningClock(Date.now()), 60_000)
    return () => window.clearInterval(timer)
  }, [])
  useEffect(() => {
    if (!gapTaskFocus) return
    const row = [...document.querySelectorAll('[data-daily-task-id]')].find(node => node.dataset.dailyTaskId === gapTaskFocus)
    if (!row) return
    row.scrollIntoView({ block: 'center', behavior: 'smooth' })
    row.querySelector('.daily-field-service select')?.focus({ preventScroll: true })
    setGapTaskFocus(null)
  }, [gapTaskFocus, teams])
  const taskMoveSavingRef = useRef(false)
  const holidayCalendar = useNationalHolidays([authUser ? String(date || '').slice(0, 4) : '', taskMove?.destinationDay?.slice(0, 4) || ''])
  ...
  const loadAgendaForDate = (nextDate, { announce = true } = {}) => {
    ...
    setDate(nextDate)
    if (String(nextDate || '') < currentLocalDate()) return
  ...
  const selectTaskService = (teamIndex, taskIndex, selectedId) => {
    ...
    const nextTask = selected
      ? { ...currentTask, ...(isTrainingService(selected) ? trainingClientPatch(currentTask.client || '') : {}), serviceId: selected.id, service: selected.name, estimatedMinutes: normalizeServiceEstimatedMinutes(selected.estimatedMinutes), estimatedMinutesCustomized: false, installationZone: serviceCode(selected) === 'alarm-installation' ? currentTask?.installationZone || '' : '' }
  ...
  const validateAgenda = (agendaTeams = teams, onlyTaskId = null) => {
    ...
    agendaTeams.forEach((team, teamIndex) => team.tasks.forEach((task, taskIndex) => {
      ...
      if (!task.time) fields.push('hora')
      if (!task.service) fields.push('tipo de servicio')
      if (!task.customerId && !task.subscriberReservation && !(isTrainingService(serviceForTask(task)) && String(task.client || '').trim())) fields.push('abonado o cliente registrado, o reserva PIG')
      ...
      if (!task.address) fields.push('dirección')
      if (!task.phone && !isTrainingService(serviceForTask(task))) fields.push('contacto')
      if (serviceCode(serviceForTask(task)) === 'alarm-installation' && !task.installationZone) fields.push('ubicación de la instalación')
      if (requiresPaymentAmount(task, serviceForTask(task))) fields.push('monto')
      if (invalidFormEmail(task, serviceForTask(task))) fields.push('email del formulario válido')
      if (fields.length) missing.push(`Equipo ${teamIndex + 1}, servicio ${taskIndex + 1}: ${fields.join(', ')}`)
  ...
  const previewDetails = task => {
    ...
    const technicianExtras = technicianVisibleServiceExtras(extras)
    const lines = [
      `Observación: ${previewValue(task.detail)}`,
      technicianExtras.paymentMethod && `Forma de pago: ${technicianExtras.paymentMethod}`,
      technicianExtras.paymentMethod && `Monto: ${previewValue(formatCurrencyAmount(technicianExtras.amount))}`,
      technicianExtras.form && `Formulario: ${technicianExtras.form}`
    ].filter(Boolean)
    return `\n📝 *Detalle:*\n${lines.join('\n')}`
  }
  const taskMessage = task => `${journeyLabel(task, history) ? journeyLabel(task, history) + '\n' : ''}🕒 ${task.time || '--:--'} Hs\n🛠️ *${task.service || 'Servicio'}*\n👤 *${task.client || 'Cliente'}*${task.awaitingConfirmation === true ? '\n🟡 *(A CONFIRMAR)*' : ''}${task.subscriberReservation ? '\n🟡 *Reserva de nuevo abonado · PIG pendiente*' : ''}${previewDetails(task)}${task.address ? `\n📍 *Dirección:* ${task.address}` : ''}${task.phone ? `\n📞 *Contacto:* ${task.phone}` : ''}`
  ...
  const registerHistory = async (agendaTeams = teams, onlyTaskId = null) => {
    ...
    try {
      ...
      const records = agendaTeams.flatMap((team, teamIndex) => team.tasks.filter(task => !task.vehicleControl && (!onlyTaskId || task.taskId === onlyTaskId)).map((task, taskIndex) => ({
        ...
        awaitingConfirmation: task.awaitingConfirmation === true,
        date, time: task.time, scheduledTime: task.time, estimatedMinutes: serviceEstimateForTask(task, serviceForTask(task)), estimatedMinutesCustomized: Boolean(task.estimatedMinutesCustomized), team: `Equipo ${teamIndex + 1}`,
        ...
        internalNote: task.internalNote || '', internalChecklist: normalizeInternalChecklist(task.internalChecklist),
      ...
      let payload
      if (onlyTaskId) {
        const team = teams.find(team => team.tasks.some(task => task.taskId === onlyTaskId))
        const task = team?.tasks.find(task => task.taskId === onlyTaskId)
  ...
  const dailyServiceHasChanges = task => {
    if (!taskHasContent(task)) return false
    const saved = historyRecordForTask(task, date, history)
    const editable = value => ({ ...value, ...applicableServiceExtras(value, serviceForTask(value)), estimatedMinutes: serviceEstimateForTask(value, serviceForTask(value)) })
    return serviceHasChanges(editable(task), saved ? editable(saved) : null)
  }
  const pendingDailyServices = teams.flatMap((team, teamIndex) => team.tasks.map((task, taskIndex) => ({ task, teamIndex, taskIndex })))
    .filter(({ task }) => !task.vehicleControl && taskHasContent(task) && dailyServiceHasChanges(task))
  const saveDailyService = async taskId => {
    if (singleSaveRef.current) return false
    singleSaveRef.current = true
    setSingleSaving(true)
    try {
      const target = teams.flatMap(team => team.tasks).findIndex(task => task.taskId === taskId)
      const row = document.querySelectorAll('.content .team-card .task-row')[target]
  ...
  useEffect(() => {
    ...
    navigationGuardRef.current = action => {
      if (singleSaveRef.current) return
  ...
  useEffect(() => {
    if (!dailyResume) return
    setDailyResume(null)
    if (pendingDailyServices.length) setDailyLeave(dailyResume)
    else dailyResume.action()
  }, [dailyResume])
  const continueDailyEditing = () => {
    const taskId = pendingDailyServices[0]?.task.taskId
    setDailyLeave(null)
    // Wait for modal removal and its automatic focus restoration before moving to the card.
    requestAnimationFrame(() => requestAnimationFrame(() => focusPendingService(taskId)))
  }
  const cancelDailyAndContinue = () => {
    if (!dailyLeave || singleSaveRef.current) return
    const first = pendingDailyServices[0]
    const request = dailyLeave
    if (first) {
      // Discard only this edit; an existing service must never become an empty slot.
      setTeams(previous => previous.map(team => ({ ...team, tasks: team.tasks.map(task => {
        if (task.taskId !== first.task.taskId) return task
        const saved = historyRecordForTask(task, date, history)
        return saved ? restoreSavedService(task, saved) : { ...blankTask(), taskId: task.taskId, time: task.time }
      }) })))
    }
    setDailyLeave(null)
    setDailyResume(request)
  }
  const saveDailyAndContinue = async () => {
    const request = dailyLeave
    const first = pendingDailyServices[0]
    setDailyLeave(null)
    if (!first) { request.action(); return }
    if (await saveDailyService(first.task.taskId)) setDailyResume(request)
  }
  ...
  const requestAgendaAction = (action, allowWithoutTechnicians = false, agendaTeams = teams, skipCustomerProposal = false) => {
    ...
    agendaTeams.forEach(team => team.tasks.forEach(task => {
      if (isTrainingService(serviceForTask(task))) return
    ...
    const conflicts = technicianTimeConflicts(agendaTeamsWithRealServices(agendaTeams), activeTechs)
    if (conflicts.length) {
  ...
  const moveDayPlan = day => {
    if (day === date) return { teams }
    const defaults = (isSaturday(day) ? [null] : weekly?._monthlyTeams?.[day.slice(0, 7)]?.teams || []).map((team, index) => ({
      ...team, teamId: team?.teamId || weekly?.[day]?.teams?.[index]?.teamId || `daily-move-${day}-${index}`,
      label: team?.label || `Equipo ${index + 1}`, members: team?.members || [], memberIds: team?.memberIds || [],
      tasks: defaultServiceTasksForDate(day, weekly)
    }))
    const stored = weekly?.[day] || {}
    const storedTeams = alignDefaultServiceTimes(stored.teams || [], day, defaultServiceTimesForDate(day, weekly), fallbackDefaultServiceTimesForDate(day))
    let destinationTeams = mergeStoredTeamsWithDefaults(defaults, storedTeams)
    destinationTeams = renumberVisibleWeeklyTeams(applyRemovedWeeklyTeams(applyRemovedWeeklySlots(applyRemovedWeeklyTasks(destinationTeams, stored.removedTaskIds || []), stored.removedSlots || []), stored.removedTeams || []))
    if (isSaturday(day)) destinationTeams = assignGuardToEmptySaturday(normalizeSaturdayTeams(destinationTeams, day, weekly), day, weekly, activeTechs)
    return { ...stored, teams: destinationTeams }
  }
  ...
  const confirmTaskMove = async () => {
    ...
    const task = sourceTeam?.tasks?.find(task => task.taskId === taskMove.taskId) || sourceTeam?.tasks?.[taskMove.taskIndex]
    const day = taskMove.destinationDay
    if (!/^\d{4}-\d{2}-\d{2}$/.test(day || '')) { setNotice('Seleccioná una fecha válida.'); return }
    const destinationTeam = moveDayPlan(day).teams[taskMove.destinationTeamIndex]
    if (!task || !destinationTeam) { setNotice('Seleccioná una fecha y un equipo de destino.'); return }
    if (day < currentLocalDate() || isSunday(day)) { setNotice('Elegí hoy o una fecha futura habilitada, excepto domingo.'); return }
    if (day === date && destinationTeam.teamId === sourceTeam.teamId) { setNotice('Elegí otro equipo o una fecha diferente.'); return }
    if (holidayCalendar.loading || holidayCalendar.error) { setNotice('Esperá a que se verifique el calendario de feriados antes de reasignar.'); return }
    ...
    if (destinationGap) { setNotice(`No se puede reasignar porque ${scheduleConflictMessage(destinationGap)}.`); return }
    const baseRecord = historyRecordForTask(task, date, history) || null
    ...
    taskMoveSavingRef.current = true
    try {
      await persistWeeklyService({ day, team: destinationTeam, task: { ...movedTask, historyId: record.id, status: record.status },
        record, baseRecord, baseTask: weekly?.[date]?.teams?.flatMap(team => team.tasks || []).find(item => item.taskId === task.taskId) || null,
        sourceDay: date, sourceTeamId: sourceTeam.teamId })
      setTaskMove(null)
      setNotice(`El servicio fue reasignado a ${record.team} para el ${prettyDate(day)}.`)
    } catch (error) { setNotice(`No se pudo reasignar el servicio. ${error.message || 'Intentá nuevamente.'}`) }
    finally { taskMoveSavingRef.current = false }
  }
  useEffect(() => {
    ...
    dateInput.type = 'date'
    dateInput.min = currentLocalDate()
    dateInput.value = taskMove.destinationDay
    ...
    const populate = day => {
      ...
      options.forEach((team, index) => {
        if (day === date && team.teamId === taskMove.sourceTeamId) return
        const option = document.createElement('option')
        option.value = String(index)
        option.textContent = `${team.label || 'Equipo ' + (index + 1)} · ${team.members?.join(' / ') || 'Sin técnicos'}`
        select.append(option)
      })
      error.textContent = select.options.length ? '' : 'No hay equipos disponibles. Elegí otra fecha o configurá sus equipos en Agenda semanal.'
    }
    populate(taskMove.destinationDay)
    if ([...select.options].some(option => Number(option.value) === taskMove.destinationTeamIndex)) select.value = String(taskMove.destinationTeamIndex)
    ...
    cancel.onclick = () => { if (!taskMoveSavingRef.current) setTaskMove(null) }
    ...
    document.body.append(layer)
    return () => layer.remove()
  }, [taskMove, teams, weekly, history, holidayCalendar.loading, holidayCalendar.error, holidayCalendar.records])
  const dailyLeaveModal = dailyLeave && <div className="modal-layer" style={{ zIndex: 10000 }}><section className="modal unsaved-service-modal" role="alertdialog" aria-modal="true" aria-label="Servicios sin guardar"><h2>Servicios sin guardar</h2><p>Hay {pendingDailyServices.length} servicio(s) de esta agenda sin guardar. Podés guardar, seguir editando o cancelar la carga del servicio pendiente. Los servicios ya guardados se conservan.</p><div className="modal-actions"><button type="button" className="secondary" onClick={continueDailyEditing}><Icon name="edit" size={16} />Seguir editando</button><button type="button" className="danger-button" onClick={cancelDailyAndContinue}><Icon name="close" size={16} />Cancelar carga</button><button type="button" className="primary" onClick={saveDailyAndContinue}><Icon name="check" size={16} />Guardar y continuar</button></div></section></div>
  const dailyCustomerField = (task, teamIndex, taskIndex) => {
    const required = taskHasContent(task)
    return isTrainingService(serviceForTask(task))
      ? <label className="daily-field-customer"><RequiredLabel required={required}>Nombre de la capacitación</RequiredLabel><BufferedInput aria-required={required} placeholder="CAPACITACIÓN CERCO ELÉCTRICO" value={task.client} onCommit={value => updateTask(teamIndex, taskIndex, trainingClientPatch(value))} /></label>
      : <DailyCustomerField task={task} customers={customers} teamIndex={teamIndex} taskIndex={taskIndex} required={required} onTextCommit={commitCustomerText} onCustomerSelect={selectCustomerResult} onAddCustomer={beginNewCustomer} onReserveSubscriber={serviceCode(serviceForTask(task)) === 'alarm-installation' ? beginSubscriberReservation : undefined} />
  }
  const gapsForDailyTeam = (team, now) => serviceGaps(team.tasks.map(task => taskWithServiceEstimate({ ...historyRecordForTask(task, date, history), ...task, date }, serviceForTask(task))), { ...planningHoursForDay(date), day: date, now })
  const dailyGaps = teams.map(team => gapsForDailyTeam(team, dailyPlanningClock))
  const addDailyGapTask = (teamIndex, start) => {
    const gap = gapsForDailyTeam(teams[teamIndex], Date.now()).find(item => item.start === start)
    if (!gap) { setDailyPlanningClock(Date.now()); setNotice('La disponibilidad cambió. Revisá los horarios antes de agregar el servicio.'); return }
    const task = { ...blankTask(), time: gap.start }
    setTeams(previous => previous.map((team, index) => index === teamIndex ? { ...team, tasks: [...team.tasks.slice(0, gap.beforeIndex), task, ...team.tasks.slice(gap.beforeIndex)] } : team))
    setGapTaskFocus(task.taskId)
  }
  if (pastDayBlocked) {
    return <>{dailyLeaveModal}<div className="module-intro"><div><p className="eyebrow">PLANIFICACIÓN DIARIA</p><h1>Jornada finalizada</h1><p>Las fechas pasadas son de solo lectura en las agendas. Consultá o corregí los servicios registrados desde Historial.</p></div></div><div className="agenda-toolbar"><label><RequiredLabel>Fecha de trabajo</RequiredLabel><input required min={currentLocalDate()} type="date" value={date} onChange={event => setDate(event.target.value)} /></label><span>{prettyDate(date)}</span></div><p className="weekly-guard-advanced"><Icon name="lock" size={16} /><span>Esta jornada ya finalizó y no admite equipos ni servicios nuevos.</span></p></>
  }
  if (sundayBlocked) {
    return <>{dailyLeaveModal}<div className="module-intro"><div><p className="eyebrow">PLANIFICACIÓN DIARIA</p><h1>Agenda del domingo bloqueada</h1><p>Los domingos son días no operativos y no admiten equipos, técnicos ni servicios.</p></div></div><div className="agenda-toolbar"><label><RequiredLabel>Fecha de trabajo</RequiredLabel><input required type="date" value={date} onChange={event => setDate(event.target.value)} /></label><span>{prettyDate(date)}</span></div><p className="weekly-guard-advanced"><Icon name="lock" size={16} /><span>Domingo sin programación. Seleccioná otro día para organizar servicios.</span></p></>
  }
  if (holidayCalendarUnavailable || holidayBlocked) {
    const closedHoliday = holiday && holidayDecision?.status === 'closed'
    return <><div className="module-intro"><div><p className="eyebrow">PLANIFICACIÓN DIARIA</p><h1>{closedHoliday ? 'Agenda del feriado bloqueada' : 'Organizá los trabajos del día'}</h1><p>{closedHoliday ? `${holiday.name} fue definido como día no operativo y no admite servicios.` : 'La disponibilidad se habilita después de verificar el calendario laboral.'}</p></div></div><div className="agenda-toolbar"><label><RequiredLabel>Fecha de trabajo</RequiredLabel><input required type="date" value={date} onChange={event => setDate(event.target.value)} /></label><span>{prettyDate(date)}</span></div>{holidayCalendar.loading ? <div className="holiday-calendar-state"><span className="loading-spinner" />Verificando feriados nacionales…</div> : holidayCalendar.error ? <div className="holiday-calendar-state error"><Icon name="alert" /><div><b>No se pudo verificar el calendario nacional</b><p>{holidayCalendar.error}</p></div><button className="secondary" onClick={() => window.location.reload()}>Reintentar</button></div> : <HolidayDecisionPanel holiday={holiday} decision={holidayDecision} canDecide={authUser?.roleCode === 'administrator'} onDecision={status => recordHolidayDecision(setWeekly, setNotice, date, holiday, status, { weekly, history, setHistory, holidays: holidayCalendar.records })} />}</>
  }
  if (advancedGuard) {
  ...
  return <>{dailyLeaveModal}<div className="module-intro"><div><p className="eyebrow">PLANIFICACIÓN DIARIA</p><h1>Organizá los trabajos del día</h1><p>Asigná técnicos y servicios para armar la agenda de cada equipo.</p></div><div className="action-group"><button className="secondary" onClick={() => setConfirmation('clear')}><Icon name="trash" />Limpiar agenda</button><button className="secondary" onClick={() => setPreview(true)}><Icon name="eye" />Vista previa</button>{showSaveAgenda && <button type="button" className="secondary save-agenda-button" onClick={saveAgenda}><Icon name="check" />Guardar agenda</button>}<button className="primary" onClick={copyAgenda}><Icon name="copy" />Copiar agenda</button></div></div><div className="agenda-toolbar"><label><RequiredLabel>Fecha de trabajo</RequiredLabel><input required type="date" value={date} onChange={event => setDate(event.target.value)} /></label><span>{prettyDate(date)}</span></div>{teams.map((team, teamIndex) => <article className="team-card" key={team.teamId || teamIndex}><div className="team-header"><div className="daily-team-heading"><span className="team-number">{teamIndex + 1}</span><div className="daily-team-identity"><strong>Equipo {teamIndex + 1}</strong><span className="daily-team-member-names" title={team.members.join(' · ') || 'Sin técnicos'}>{team.members.length ? team.members.map(name => String(name).trim().split(/\s+/)[0]).join(' · ') : 'Sin técnicos'}</span></div>{teams.length > 1 && <button className="team-delete" onClick={() => setConfirmation({ type: 'team', index: teamIndex })}><Icon name="trash" size={16} />Eliminar equipo</button>}</div><div className="technicians-picker"><span className="technician-assignment-label"><RequiredLabel>{team.members.length ? `${team.members.length} técnico(s) asignado(s)` : 'Sin técnicos asignados'}</RequiredLabel></span><button className="secondary small" onClick={() => { setTechOpen(techOpen === teamIndex ? null : teamIndex); setFilter('') }}><Icon name="users" size={16} />Agregar técnicos</button>{techOpen === teamIndex && <div className="tech-popover"><input autoFocus placeholder="Buscar técnico..." value={filter} onChange={event => setFilter(event.target.value)} /><div className="tech-list">{activeTechs.filter(tech => tech.name.toLowerCase().includes(filter.toLowerCase())).map(tech => <label key={tech.id}><input type="checkbox" checked={(team.memberIds || []).some(id => String(id) === String(tech.id))} onChange={() => toggleTech(teamIndex, tech)} />{tech.name}</label>)}</div></div>}</div></div><div className="tasks">{team.tasks.map((task, taskIndex) => <React.Fragment key={task.taskId || task.historyId || taskIndex}>{dailyGaps[teamIndex].filter(gap => gap.beforeIndex === taskIndex).map(gap => <div className="weekly-time-gap daily-time-gap" key={gap.start}><strong>Disponibilidad horaria desde las {gap.start} hs hasta las {gap.end} hs</strong><button type="button" className="secondary" onClick={() => addDailyGapTask(teamIndex, gap.start)}><Icon name="plus" size={16} />Agregar servicio</button></div>)}<div className="task-row" data-daily-task-id={task.taskId} tabIndex={-1} key={task.taskId || task.historyId || `${team.teamId || teamIndex}-${taskIndex}`}><div className="task-title"><span>{taskIndex + 1}</span><b>{task.vehicleControl ? 'Control vehicular' : 'Servicio'}</b></div>{task.vehicleControl ? <div className="vehicle-control-daily-fields"><span><b>Hora:</b> {task.time} hs</span><span><b>Tipo:</b> {task.service}</span><span><b>Duración:</b> 15 minutos</span><span><b>Vehículo:</b> {task.client || vehicleLabel(task)}</span><span><b>Técnico responsable:</b> {task.technicians?.[0] || team.members?.[0] || 'Sin asignar'}</span></div> : <><label className="daily-field-time"><RequiredLabel>Hora</RequiredLabel><input aria-required="true" type="time" value={task.time} onChange={event => updateTask(teamIndex, taskIndex, { time: event.target.value })} /></label><label className="daily-field-service"><RequiredLabel>Tipo de servicio</RequiredLabel><select aria-required="true" value={serviceForTask(task)?.id || ''} onChange={event => selectTaskService(teamIndex, taskIndex, event.target.value)}><option value="">Seleccionar</option>{activeServices.map(service => <option key={service.id} value={service.id}>{service.name}</option>)}</select></label>
<div className="daily-field-duration"><ServiceEstimatedDurationField required={taskHasContent(task)} value={serviceEstimateForTask(task, serviceForTask(task))} onChange={estimatedMinutes => updateTask(teamIndex, taskIndex, { estimatedMinutes, estimatedMinutesCustomized: true })} />{journeyLabel(historyRecordForTask(task, date, history), history) && <small className="journey-label">{journeyLabel(historyRecordForTask(task, date, history), history)}</small>}</div>
...
{!task.vehicleControl && <ServicePhotoManager record={{ ...(historyRecordForTask(task, date, history) || {}), ...task }} onMetadataChange={patch => { updateTask(teamIndex, taskIndex, patch); setHistory(previous => previous.map(record => String(record.id) === String(task.historyId) || String(record.sourceTaskId) === String(task.taskId) ? { ...record, ...patch } : record)) }} />}
{dailyCustomerField(task, teamIndex, taskIndex)}
<label className="daily-field-address"><RequiredLabel>Dirección</RequiredLabel><input aria-required="true" value={task.address} onChange={event => updateTask(teamIndex, taskIndex, { address: event.target.value })} /></label><label className="daily-field-contact">{isTrainingService(serviceForTask(task)) ? 'Contacto (opcional)' : <RequiredLabel>Contacto</RequiredLabel>}<input aria-required={!isTrainingService(serviceForTask(task))} value={task.phone} onChange={event => updateTask(teamIndex, taskIndex, { phone: event.target.value })} /></label><label className="observations daily-field-observations">Observaciones<BufferedTextarea value={task.detail} onCommit={value => updateTask(teamIndex, taskIndex, { detail: value })} /></label>{serviceCode(serviceForTask(task)) === 'alarm-installation' && <fieldset className="installation-zone"><legend><RequiredLabel>Ubicación de la instalación</RequiredLabel></legend>{INSTALLATION_ZONES.map(([value, label]) => <label key={value}><input type="radio" aria-required="true" name={`zone-${teamIndex}-${taskIndex}`} checked={task.installationZone === value} onChange={() => { const nextTask = { ...task, installationZone: value }; updateTask(teamIndex, taskIndex, { installationZone: value, ...applicableServiceExtras(nextTask, serviceForTask(task)) }) }} />{label}</label>)}</fieldset>}<ServiceExtraFields className="daily-extra-fields" task={task} service={serviceForTask(task)} buffered onChange={patch => updateTask(teamIndex, taskIndex, patch)} /></>}<div className="daily-task-actions">{!task.vehicleControl && <>{dailyServiceHasChanges(task) && <button type="button" className="primary daily-save-button" disabled={singleSaving} onClick={() => saveDailyService(task.taskId)}><Icon name="check" size={16} /><span>{singleSaving ? 'Guardando…' : 'Guardar'}</span></button>}<ServiceJourneys record={historyRecordForTask(task, date, history)} disabled={dailyServiceHasChanges(task)} action /><ServiceConfirmationButton task={task} day={date} history={history} persist={persistWeeklyService} disabled={Boolean(historyRecordForTask(task, date, history)) && dailyServiceHasChanges(task)} onDraftChange={!historyRecordForTask(task, date, history) ? patch => updateTask(teamIndex, taskIndex, patch) : undefined} /><button type="button" className="icon-btn daily-copy-button" title="Copiar este servicio" aria-label={`Copiar servicio ${taskIndex + 1} del Equipo ${teamIndex + 1}`} onClick={() => copySingleTask(task, team, teamIndex, taskIndex)}><Icon name="copy" size={16} /><span>Copiar</span></button>{taskHasContent(task) && <button type="button" className="icon-btn move daily-move-button" title="Reasignar a otro equipo" aria-label={`Reasignar servicio ${taskIndex + 1} a otro equipo`} onClick={() => openTaskMove(teamIndex, taskIndex)}><span aria-hidden="true">⇄</span><span>Reasignar</span></button>}{(team.tasks.length > 1 || taskHasContent(task)) && <button type="button" className="icon-btn delete daily-delete-button" title="Eliminar servicio" aria-label={`Eliminar servicio ${taskIndex + 1} del Equipo ${teamIndex + 1}`} onClick={() => setConfirmation({ type: 'task', journey: Boolean(task.serviceJourney || historyRecordForTask(task, date, history)?.serviceJourney), client: task.client || 'Servicio sin cliente', action: async () => { try { await persistWeeklyService({ operation: 'task-remove', day: date, teamId: team.teamId, teamIndex, taskIndex, taskId: task.taskId, historyId: task.historyId, time: task.time, wasPlaceholder: !taskHasContent(task), fallbackPlan: { teams } }); setNotice('El servicio fue eliminado correctamente.') } catch (error) { setNotice(`No se eliminó el servicio. ${error.message}`) } } })}><Icon name="trash" size={16} /><span>Eliminar</span></button>}</>}</div></div></React.Fragment>)}{dailyGaps[teamIndex].filter(gap => gap.beforeIndex === team.tasks.length).map(gap => <div className="weekly-time-gap daily-time-gap" key={gap.start}><strong>Disponibilidad horaria desde las {gap.start} hs hasta las {gap.end} hs</strong><button type="button" className="secondary" onClick={() => addDailyGapTask(teamIndex, gap.start)}><Icon name="plus" size={16} />Agregar servicio</button></div>)}</div><button className="link-button" onClick={() => setTeams(previous => previous.map((item, index) => index === teamIndex ? { ...item, tasks: [...item.tasks, blankTask()] } : item))}><Icon name="plus" size={16} />Agregar servicio</button></article>)}<button className="add-team" onClick={() => setTeams([...teams, { teamId: createTeamId(), memberIds: [], members: [], tasks: [blankTask()] }])}><Icon name="plus" />Agregar otro equipo</button>{confirmation?.type === 'task' && <Confirm title="Eliminar servicio" detail={`¿Querés eliminar ${confirmation.client} de esta agenda? Revisá el cliente antes de confirmar.${confirmation.journey ? ' Si la jornada está pendiente, quedará cancelada. Se conservarán su historial y las otras jornadas.' : ''}`} destructive action={confirmation.action} close={() => setConfirmation(null)} />}{preview && <Preview title="Vista previa de la agenda" text={message} onCopy={copyAgenda} close={() => setPreview(false)} />}{confirmation === 'clear' && <Confirm title="Limpiar agenda" detail="¿Querés borrar todos los equipos y servicios cargados?" destructive action={clearAgenda} close={() => setConfirmation(null)} />}{confirmation?.type === 'team' && <Confirm title="Eliminar equipo" detail={`¿Querés eliminar el Equipo ${confirmation.index + 1}? Se quitará el equipo y su asignación de técnicos de ese día. Si sólo tiene controles vehiculares pendientes, un administrador también los omitirá porque esos vehículos no se utilizarán. Otros servicios o controles completados impiden eliminarlo.`} destructive action={async () => { const target = teams[confirmation.index]; if (!target) return; try { await persistWeeklyService({ operation: 'team-remove', day: date, teamId: target.teamId, teamIndex: confirmation.index, fallbackPlan: { teams } }); setTechOpen(null); setNotice('El equipo y su asignación fueron eliminados; sus controles vehiculares pendientes, si los había, fueron omitidos.'); } catch (error) { setNotice(`No se eliminó el equipo. ${error.message}`); } }} close={() => setConfirmation(null)} />}</>
}
 ...
 */
function WeeklyPlanner({ navigationGuardRef, persistWeeklyService, persistWeeklyConfiguration, weekly, setWeekly, customers, setCustomers, services, activeTechs, history, setHistory, setNotice, openDaily, authUser, vehicles, permissions = {} }) {
  ...
  const canConfigureWeekly = key => isAdministrator || Boolean(permissions[key])
  ...
  const [taskMove, setTaskMove] = useState(null)
  const [taskMoveSaving, setTaskMoveSaving] = useState(false)
  const taskMoveSaveGuardRef = useRef(false)
  ...
  const openTaskEditor = (day, teamIndex, taskIndex) => {
    ...
    if (selectedTask && ['Completado', 'Avance registrado'].includes(taskStatus(selectedTask, day, operationalHistory))) {
  ...
  const saveTaskEditor = async () => {
    ...
    const photoPatch = taskEditor.photoData
      ? { servicePhotoAttached: true, servicePhotoUrl: `/api/service-photo/${encodeURIComponent(String(previousRecord?.id || tracedDraft.historyId || `work-${tracedDraft.taskId}`))}`, servicePhotoAttachedAt: new Date().toISOString() }
      : taskEditor.photoRemoved ? { servicePhotoAttached: false, servicePhotoUrl: '', servicePhotoAttachedAt: '' } : {}
    const record = { ...previousRecord, ...weeklyHistoryRecord(day, savedTeam, teamIndex, { ...tracedDraft, ...photoPatch }, taskIndex), ...photoPatch, status: previousRecord?.status || 'Pendiente' }
    if (previousRecord) record.id = previousRecord.id
    const savedTask = { ...tracedDraft, ...photoPatch, historyId: record.id, status: record.status }
    try {
  ...
  const requestWeeklyLeave = action => {
    if (taskEditorSaveGuardRef.current) return
    const dirtyEditor = taskEditor && (
    ...
    if (dirtyEditor) { setLeaveRequest({ action }); return }
    for (const day of Object.keys(weekly).filter(key => /^\d{4}-\d{2}-\d{2}$/.test(key)).sort()) {
      const plan = dayPlan(day)
      for (let teamIndex = 0; teamIndex < plan.teams.length; teamIndex++) {
        const taskIndex = plan.teams[teamIndex].tasks.findIndex(task => taskHasContent(task) && !task.vehicleControl &&
          String(task.createdBy?.id || '') === String(authUser?.id || '') && authUser?.id &&
          !historyRecordForTask(task, day, operationalHistory))
        if (taskIndex >= 0 && !dayHasFinished(day)) {
          openTaskEditor(day, teamIndex, taskIndex)
  ...
  useEffect(() => {
    ...
    setResumeNavigation(null)
    requestWeeklyLeave(resumeNavigation.action)
  }, [resumeNavigation])
  const saveAndLeave = async () => {
    const request = leaveRequest
    setLeaveRequest(null)
    if (await saveTaskEditor()) setResumeNavigation(request)
  }
  const discardAndLeave = () => {
    if (!leaveRequest || taskEditorSaveGuardRef.current) return
    const { action } = leaveRequest
    // Only abandon the editor's local draft. Never delete a persisted service,
    // its history, or changes received from another session.
    setLeaveRequest(null)
    setTaskEditor(null)
    action()
  }
  ...
  const openWeeklyTaskMove = (event, day, teamIndex, taskIndex) => {
    ...
    taskMoveSaveGuardRef.current = false
    setTaskMoveSaving(false)
    setTaskMove({
      day,
      destinationDay: day,
  ...
  const selectWeeklyTaskMoveDate = destinationDay => {
    const destinationPlan = /^\d{4}-\d{2}-\d{2}$/.test(destinationDay) ? dayPlan(destinationDay) : { teams: [] }
    setTaskMove(previous => {
      if (!previous) return previous
      const firstDestination = destinationPlan.teams.findIndex((team, index) => destinationDay !== previous.day || (
        String(team.teamId || '') !== String(previous.sourceTeamId || '') && index !== previous.sourceTeamIndex
      ))
      return { ...previous, destinationDay, destinationTeamIndex: firstDestination >= 0 ? firstDestination : 0 }
    })
  }
  const confirmWeeklyTaskMove = async () => {
    if (!taskMove || taskMoveSaveGuardRef.current) return
    const { day, destinationDay, sourceTeamIndex, sourceTeamId, taskIndex, taskId, historyId, destinationTeamIndex } = taskMove
    const sourcePlan = dayPlan(day)
    ...
    if (!movedTask || !destinationTeam) {
      setNotice('No se encontró el servicio o el equipo de destino. Recargá la planificación e intentá nuevamente.')
      return
    }
    if (destinationDay === day && String(destinationTeam.teamId || '') === String(sourceTeam?.teamId || '')) {
      setNotice('Elegí otro equipo o una fecha diferente para reasignar el servicio.')
      return
    }
    if (destinationDay < today) { setNotice('Elegí la fecha de hoy o una fecha futura.'); return }
    const destinationHours = hoursForDay(destinationDay)
    if (!destinationHours) { setNotice('Los domingos no están habilitados para programar servicios.'); return }
    const destinationHoliday = holidayStateForDay(destinationDay)
    if (holidayCalendar.loading) { setNotice('Esperá mientras se verifica el calendario de feriados.'); return }
    if (holidayCalendar.error) { setNotice('No se pudo verificar el calendario de feriados. Reintentá antes de mover el servicio.'); return }
    ...
    const destinationTasks = (destinationTeam.tasks || []).filter(task => !matchesTask(task) && !(
      task.time === movedTask.time && !String(task.client || '').trim() && !String(task.service || '').trim()
    ))
    let swapTask = null
    try { swapTask = vehicleControlSwapCandidate(movedTask, destinationTeam, day, destinationDay) }
    catch (error) { setNotice(error.message); return }
    const destinationConflict = minimumServiceGapConflicts([{ ...destinationTeam, tasks: [...destinationTasks.filter(task => task !== swapTask), movedTask].map(task => {
      const occupancyTask = taskForScheduleOccupancy(task, destinationDay, operationalHistory)
      return occupancyTask ? taskWithServiceEstimate(occupancyTask, serviceForWeeklyTask(task)) : null
    }).filter(Boolean) }])[0]
    if (destinationConflict) { setNotice(planningConflictMessage(destinationDay, destinationTeam, destinationTeamIndex, destinationConflict)); return }
    const baseRecord = historyRecordForTask(movedTask, day, operationalHistory) || null
    const baseTask = weekly?.[day]?.teams?.flatMap(team => team.tasks || []).find(matchesTask) || null
    const destinationRecordData = weeklyHistoryRecord(destinationDay, destinationTeam, destinationTeamIndex, movedTask, destinationTasks.length)
    const changedDate = destinationDay !== day
    const record = stampServiceRecord({
      ...baseRecord,
      ...destinationRecordData,
      id: baseRecord?.id || destinationRecordData.id,
      status: changedDate ? 'Pendiente' : (baseRecord?.status || movedTask.status || 'Pendiente'),
      scheduledDate: '',
      ...(changedDate ? { rescheduledFrom: day, reprogrammedAt: new Date().toISOString() } : {})
    }, authUser)
    const savedTask = { ...movedTask, historyId: record.id, status: record.status }
    taskMoveSaveGuardRef.current = true
    setTaskMoveSaving(true)
    try {
      await persistWeeklyService({
        ...
        baseTask,
        sourceDay: day,
        swapTaskId: swapTask?.taskId || '',
        sourceTeamId: sourceTeam.teamId
      })
      setTaskMove(null)
      if (changedDate) setAnchor(destinationDay)
      setNotice(changedDate
        ? `El servicio fue reprogramado para el ${prettyDate(destinationDay)} en ${destinationTeam.label || `Equipo ${destinationTeamIndex + 1}`}.`
        : `El servicio fue reasignado a ${destinationTeam.label || `Equipo ${destinationTeamIndex + 1}`} y se conservó toda su información.`)
    } catch (error) {
      setNotice(`No se pudo reasignar el servicio. ${error.message || 'Comprobá la conexión e intentá nuevamente.'}`)
    } finally {
      taskMoveSaveGuardRef.current = false
      setTaskMoveSaving(false)
    }
  ...
  const removeWeeklyTeam = async (day, teamIndex) => {
    ...
    try {
      ...
      setTeamRemoval(null)
      setTechPicker(null)
      setNotice('El equipo y su asignación fueron eliminados; sus controles vehiculares pendientes, si los había, fueron omitidos.')
    } catch (error) {
      setTeamRemoval(null)
      setNotice(`No se eliminó el equipo. ${error.message || 'Revisá la versión actual e intentá nuevamente.'}`)
    }
  }
  const hoursForDay = planningHoursForDay
  ...
  const updateVehicleControlOverride = (vehicleId, friday, technicianId) => setMonthlyVehicleSetup(previous => ({
    ...previous,
    assignments: previous.assignments.map(item => {
      if (String(item.vehicleId) !== String(vehicleId)) return item
      const weeklyOverrides = { ...(item.weeklyOverrides || {}) }
      if (!technicianId || String(technicianId) === String(item.technicianId)) delete weeklyOverrides[friday]
      else weeklyOverrides[friday] = technicianId
      return { ...item, weeklyOverrides }
    })
  }))
  const saveMonthlyVehicleSetup = async () => {
    ...
    const duplicatedOverride = monthFridays(monthlyVehicleSetup.month).filter(friday => friday >= today).find(friday => {
      const technicians = assignments.map(item => String(item.weeklyOverrides?.[friday] || item.technicianId || '')).filter(Boolean)
      return activeTechs.length >= vehicles.length && new Set(technicians).size !== technicians.length
    })
    if (duplicatedOverride) { setNotice(`El ${prettyDate(duplicatedOverride)} un técnico quedó asignado a más de un control a las 15:45. Elegí responsables diferentes.`); return }
    ...
    const updateWeekly = previous => {
      let next = { ...previous }
      Object.entries(next).forEach(([day, plan]) => {
        if (day.startsWith('_') || !plan?.teams) return
        next[day] = {
          ...plan,
          teams: plan.teams.map(team => ({
            ...team,
            tasks: (team.tasks || []).filter(task => !(task.vehicleControl && (
              task.monthlyVehicleAssignment === monthlyVehicleSetup.month ||
              (!task.monthlyVehicleAssignment && day.startsWith(`${monthlyVehicleSetup.month}-`))
            )))
          }))
        }
      })
    ...
    try {
      await persistWeeklyConfiguration(snapshot => ({
        ...snapshot,
        history: updateHistory(snapshot.history || []),
        agenda: { ...snapshot.agenda, weekly: updateWeekly(snapshot.agenda?.weekly || {}) }
      }))
      setMonthlyVehicleSetup(null)
      setNotice('Se guardaron los responsables y se generaron los controles de los viernes a las 15:45 hs.')
    } catch (error) {
      setNotice(`No se guardaron los responsables. ${error.message || 'Revisá la versión actual e intentá nuevamente.'}`)
    }
  }
  ...
  const openAnnualGuardSetup = () => {
    if (!canConfigureWeekly('weeklyGuards')) { setNotice('No tenés permiso para definir las guardias anuales.'); return }
  ...
  const openDay = day => {
    if (dayHasFinished(day)) { openDaily(day, []); return }
    ...
    const incompleteTask = scheduledTasks.find(({ task }) => !task.vehicleControl && (!task.time || !task.service || (!task.customerId && !task.subscriberReservation && !(isTrainingService(serviceForWeeklyTask(task)) && String(task.client || '').trim())) || !task.address || (!task.phone && !isTrainingService(serviceForWeeklyTask(task))) || !task.detail || (serviceCode(serviceForWeeklyTask(task)) === 'alarm-installation' && !task.installationZone) || requiresPaymentAmount(task, serviceForWeeklyTask(task))))
    if (incompleteTask) {
      const { task, teamIndex, taskIndex } = incompleteTask
      const missing = [['hora', task.time], ['tipo de servicio', task.service], ['cliente', task.client], ['dirección', task.address], ['contacto', isTrainingService(serviceForWeeklyTask(task)) || task.phone], ['detalle', task.detail]].filter(([, value]) => !String(value || '').trim()).map(([label]) => label)
  ...
  const weeklyHistoryRecord = (day, team, teamIndex, task, taskIndex) => ({
    ...
    estimatedMinutes: serviceEstimateForTask(task, serviceForWeeklyTask(task)),
    estimatedMinutesCustomized: Boolean(task.estimatedMinutesCustomized),
    team: team.label || `Equipo ${teamIndex + 1}`,
    teamId: team.teamId,
    technicianIds: task.vehicleControl && task.technicianIds?.length ? task.technicianIds : team.memberIds || [],
    technicians: task.vehicleControl && task.technicians?.length ? task.technicians : team.members || [],
    ...
    internalChecklist: normalizeInternalChecklist(task.internalChecklist),
    ...applicableServiceExtras(task, serviceForWeeklyTask(task)),
    address: task.address,
    phone: task.phone,
    installationZone: task.installationZone || '',
    servicePhotoAttached: Boolean(task.servicePhotoAttached),
    servicePhotoUrl: task.servicePhotoUrl || '',
    servicePhotoAttachedAt: task.servicePhotoAttachedAt || '',
    ...serviceTrace(task)
  })
  ...
  return <>
    ...
    {teamRemoval && <Confirm title="Quitar equipo" detail={`¿Querés quitar ${teamRemoval.label} de la planificación del ${prettyDate(teamRemoval.day)}? Se quitará el equipo y su asignación de técnicos de ese día. Si sólo tiene controles vehiculares pendientes, un administrador también los omitirá porque esos vehículos no se utilizarán. Otros servicios o controles completados impiden eliminarlo.`} destructive action={() => removeWeeklyTeam(teamRemoval.day, teamRemoval.teamIndex)} close={() => setTeamRemoval(null)} />}
    {taskRemoval && <Confirm title={taskRemoval.vehicleControl ? 'Omitir control vehicular' : 'Eliminar servicio'} detail={taskRemoval.vehicleControl ? `¿Querés omitir este control para el ${prettyDate(taskRemoval.day)}? El técnico no tendrá que realizarlo y no se volverá a generar al guardar la configuración mensual.` : `¿Querés eliminar el Servicio ${taskRemoval.taskIndex + 1} de ${taskRemoval.label} para el ${prettyDate(taskRemoval.day)}?${taskRemoval.journey ? ' Si la jornada está pendiente, quedará cancelada. Se conservarán su historial y las otras jornadas.' : taskRemoval.historyId ? ' También se quitará el registro pendiente vinculado.' : ''}`} destructive action={() => removeWeeklyTask(taskRemoval)} close={() => setTaskRemoval(null)} />}
    {taskMove && (() => {
      ...
      const validDestinationDay = /^\d{4}-\d{2}-\d{2}$/.test(taskMove.destinationDay || '')
      const destinationPlan = validDestinationDay ? dayPlan(taskMove.destinationDay) : { teams: [] }
    ...
    {taskEditor && (() => {
      ...
      return <><div className="modal-backdrop weekly-editor-backdrop" onMouseDown={() => { if (!taskEditorSaving) requestWeeklyLeave(() => setTaskEditor(null)) }}><section key={`${day}-${taskEditor.taskId}-${teamIndex}-${taskIndex}`} className="modal weekly-task-modal" role="dialog" aria-modal="true" aria-label={`Servicio ${taskIndex + 1}`} onMouseDown={event => event.stopPropagation()}><button type="button" className="modal-close" aria-label="Cerrar edición del servicio" title="Cerrar" onClick={() => { if (!taskEditorSaving) requestWeeklyLeave(() => setTaskEditor(null)) }}><Icon name="close" size={18} /></button><p className="eyebrow">AGENDA SEMANAL · {prettyDate(day)}</p><h2>{task.vehicleControl ? 'Control semanal de vehículo' : `Servicio ${taskIndex + 1}`}</h2><p className="weekly-modal-team">{taskEditor.teamSnapshot?.label || `Equipo ${teamIndex + 1}`} · {taskEditor.teamSnapshot?.members?.join(' / ') || 'Sin técnicos asignados'}</p><div className="weekly-task-form"><div className="week-task-top"><label><RequiredLabel>Hora</RequiredLabel><BufferedInput aria-required="true" type="time" min={hours.min} max={hours.max} value={task.time} onCommit={value => updateTaskDraft({ time: value })} /></label><label><RequiredLabel>Tipo de servicio</RequiredLabel><select aria-required="true" disabled={task.vehicleControl} value={serviceForWeeklyTask(task)?.id || ''} onChange={event => selectDraftService(event.target.value)}>{task.vehicleControl && serviceForWeeklyTask(task) ? <option value={serviceForWeeklyTask(task).id}>{serviceForWeeklyTask(task).name}</option> : <><option value="">Seleccionar</option>{activeServices.map(service => <option key={service.id} value={service.id}>{service.name}</option>)}</>}</select></label></div>{task.vehicleControl ? <label className="vehicle-control-fixed-duration"><span><RequiredLabel>Tiempo estimado</RequiredLabel></span><input value="15 minutos" readOnly /></label> : <ServiceEstimatedDurationField value={serviceEstimateForTask(task, serviceForWeeklyTask(task))} onChange={estimatedMinutes => updateTaskDraft({ estimatedMinutes, estimatedMinutesCustomized: true })} />}{task.time && <small className="task-occupied-range">Franja estimada: {taskOccupiedTimeLabel(taskWithServiceEstimate(task, serviceForWeeklyTask(task)))}</small>}{taskConflict && <p className="task-schedule-alert" role="alert"><Icon name="alert" size={16} /><span>{scheduleConflictForTaskMessage(taskConflict, taskIndex)}</span></p>}{task.vehicleControl ? <div className="vehicle-control-planning-fields"><label>Vehículo<input value={task.client || vehicleLabel(task)} readOnly /></label><label><RequiredLabel>Técnico a cargo</RequiredLabel><select value={task.technicianIds?.[0] || ''} onChange={event => { const technician = activeTechs.find(item => String(item.id) === event.target.value); updateTaskDraft({ technicianIds: technician ? [technician.id] : [], technicians: technician ? [technician.name] : [] }) }}><option value="">Seleccionar técnico</option>{activeTechs.map(technician => <option key={technician.id} value={technician.id}>{technician.name}</option>)}</select></label><p>Podés reemplazar al responsable para este control ante una contingencia. La asignación mensual predeterminada no se modifica.</p></div> : <>{serviceCode(serviceForWeeklyTask(task)) === 'alarm-installation' && <fieldset className="installation-zone weekly-installation-zone"><legend><RequiredLabel>Ubicación de la instalación</RequiredLabel></legend>{INSTALLATION_ZONES.map(([value, label]) => <label key={value}><input aria-required="true" type="radio" name={`weekly-zone-${day}-${teamIndex}-${taskIndex}`} checked={task.installationZone === value} onChange={() => { const nextTask = { ...task, installationZone: value }; updateTaskDraft({ installationZone: value, ...applicableServiceExtras(nextTask, serviceForWeeklyTask(task)) }) }} />{label}</label>)}</fieldset>}
{isTrainingService(serviceForWeeklyTask(task)) ? <label className="weekly-customer-search"><RequiredLabel>Nombre de la capacitación</RequiredLabel><BufferedInput aria-required="true" placeholder="CAPACITACIÓN CERCO ELÉCTRICO" value={task.client} onCommit={value => updateTaskDraft(trainingClientPatch(value))} /></label> : <CustomerAutocomplete awaitingConfirmation={task.awaitingConfirmation === true} className="weekly-customer-search" value={task.client} customerId={task.customerId} customers={customers} subscriberReservation={task.subscriberReservation} onTextCommit={commitDraftCustomerText} onCustomerSelect={selectDraftCustomer} onAddCustomer={beginDraftNewCustomer} onReserveSubscriber={serviceCode(serviceForWeeklyTask(task)) === 'alarm-installation' ? beginDraftSubscriberReservation : undefined} />}
<label><RequiredLabel>Dirección</RequiredLabel><BufferedInput aria-required="true" readOnly={!task.newCustomer && !task.subscriberReservation && !isTrainingService(serviceForWeeklyTask(task))} title={task.newCustomer || task.subscriberReservation || isTrainingService(serviceForWeeklyTask(task)) ? '' : 'Este dato se modifica desde Abonados y clientes'} value={task.address} onCommit={value => updateTaskDraft({ address: value })} /></label><label>{isTrainingService(serviceForWeeklyTask(task)) ? 'Contacto (opcional)' : <RequiredLabel>Contacto</RequiredLabel>}<BufferedInput aria-required={!isTrainingService(serviceForWeeklyTask(task))} readOnly={!task.newCustomer && !task.subscriberReservation && !isTrainingService(serviceForWeeklyTask(task))} title={task.newCustomer || task.subscriberReservation || isTrainingService(serviceForWeeklyTask(task)) ? '' : 'Este dato se modifica desde Abonados y clientes'} value={task.phone} onCommit={value => updateTaskDraft({ phone: value })} /></label><p className="weekly-customer-data-note">{isTrainingService(serviceForWeeklyTask(task)) ? 'Ingresá el nombre y la dirección de la capacitación. El contacto es opcional y no se creará un CLI.' : task.subscriberReservation ? 'Estos datos son provisorios y no crean un CLI. Vinculá el PIG importado cuando esté disponible.' : task.newCustomer ? 'Completá dirección y contacto para crear el cliente CLI al guardar.' : 'Dirección y contacto se administran desde el módulo Abonados y clientes.'}</p><label><RequiredLabel>Detalle</RequiredLabel><BufferedTextarea aria-required="true" value={task.detail} onCommit={value => updateTaskDraft({ detail: value })} /></label><ServiceExtraFields className="weekly-extra-fields" task={task} service={serviceForWeeklyTask(task)} buffered onChange={updateTaskDraft} /><ServicePhotoField record={task} photoData={taskEditor.photoData} removed={taskEditor.photoRemoved} disabled={taskEditorSaving} onChange={({ photoData, removed }) => setTaskEditor(previous => ({ ...previous, photoData, photoRemoved: removed }))} /></>}</div><div className="modal-actions"><ServiceConfirmationButton task={task} day={day} history={operationalHistory} onDraftChange={updateTaskDraft} disabled={taskEditorSaving} /><button className="secondary" disabled={taskEditorSaving} onClick={() => { if (!taskEditorSaving) requestWeeklyLeave(() => setTaskEditor(null)) }}><Icon name="close" size={16} />Cancelar</button><button className="primary" disabled={taskEditorSaving} onClick={saveTaskEditor}><Icon name="check" size={16} />{taskEditorSaving ? 'Guardando…' : 'Guardar servicio'}</button></div></section></div>{leaveRequest && <div className="modal-layer" style={{ zIndex: 10000 }}><section className="modal unsaved-service-modal" role="alertdialog" aria-modal="true" aria-label="Servicio sin guardar"><h2>Servicio sin guardar</h2><p>El servicio que estabas cargando tiene cambios sin guardar. Podés guardar y continuar, seguir editando o cancelar la carga de esta edición. Cancelar la carga no elimina un servicio ya guardado.</p><div className="modal-actions"><button type="button" className="secondary" onClick={() => setLeaveRequest(null)}><Icon name="edit" size={16} />Seguir editando</button><button type="button" className="danger-button" onClick={discardAndLeave} title="Cancelar carga y continuar"><Icon name="close" size={16} />Cancelar carga</button><button type="button" className="primary" onClick={saveAndLeave} title="Guardar cambios y continuar"><Icon name="check" size={16} />Guardar y continuar</button></div></section></div>}</>
    })()}
            ...
            <fieldset className="week-teams weekly-day-fields" disabled={finishedDay}>{plan.teams.map((team, teamIndex) => {
              ...
              return <article className="week-team" key={team.teamId || teamIndex}>
                ...
                {team.tasks.map((task, taskIndex) => <React.Fragment key={task.taskId || taskIndex}>{gaps.filter(gap => gap.beforeIndex === taskIndex).map(gap => <div className="weekly-time-gap" key={gap.start}><strong>Disponibilidad horaria desde las {gap.start} hs hasta las {gap.end} hs</strong>{!advancedGuard && <button type="button" className="secondary" onClick={() => addTask(day, teamIndex, gap.start)}><Icon name="plus" size={16} />Agregar servicio</button>}</div>)}<div className={`week-task week-task-summary ${!task.client ? 'available-slot' : ''}`} data-weekly-search={task.client || ''} data-weekly-search-key={`${day}/${teamIndex}/${taskIndex}`} role="button" tabIndex={0} onClick={() => openTaskEditor(day, teamIndex, taskIndex)} onKeyDown={event => { if (event.key === 'Enter' || event.key === ' ') { event.preventDefault(); openTaskEditor(day, teamIndex, taskIndex) } }}>
                  ...
                  {journeyLabel(historyRecordForTask(task, day, operationalHistory), operationalHistory) && <small className="journey-label">{journeyLabel(historyRecordForTask(task, day, operationalHistory), operationalHistory)}</small>}
                  ...
                  {task.vehicleControl ? <div className="vehicle-control-summary"><span><b>Duración:</b> 15 minutos · {taskOccupiedInterval(task)?.startTime}–{taskOccupiedInterval(task)?.serviceEndTime}</span><span><b>Técnico responsable:</b> {task.technicians?.[0] || 'Sin asignar'}</span><small>Abrí la tarjeta para cambiar excepcionalmente el técnico de este control.</small></div> : <>
                ...
                {gaps.filter(gap => gap.beforeIndex === team.tasks.length).map(gap => <div className="weekly-time-gap" key={gap.start}><strong>Disponibilidad horaria desde las {gap.start} hs hasta las {gap.end} hs</strong>{!advancedGuard && <button type="button" className="secondary" onClick={() => addTask(day, teamIndex, gap.start)}><Icon name="plus" size={16} />Agregar servicio</button>}</div>)}
...
function TechnicianPortal({ user, history, setHistory, vehicles = [], setVehicles, logout, sessionInvalidated, onBack }) {
  ...
  setVehicleControlAssignedRecords(assignedServices)
  ...
  useEffect(() => {
    const frame = requestAnimationFrame(() => {
      document.querySelectorAll('.technician-service').forEach((card, index) => {
        const record = services[index]
        const url = servicePhotoUrl(record)
        card.querySelector('.technician-service-photo')?.remove()
        if (!url) return
        const link = document.createElement('a'); link.className = 'service-photo technician-service-photo'; link.href = url; link.target = '_blank'; link.rel = 'noreferrer'; link.title = 'Abrir foto de referencia'
        const photo = document.createElement('img'); photo.src = url; photo.alt = `Foto de referencia de ${record.client || 'servicio'}`; photo.onerror = () => link.remove()
        link.append(photo)
        card.querySelector('.tech-client')?.insertAdjacentElement('afterend', link)
      })
    })
    return () => cancelAnimationFrame(frame)
  }, [services.map(record => `${record.id}:${record.servicePhotoAttachedAt || ''}`).join('|')])
  useEffect(() => {
    const refreshClock = () => setClock(Date.now())
    const interval = window.setInterval(refreshClock, 30000)
    window.addEventListener('focus', refreshClock)
    document.addEventListener('visibilitychange', refreshClock)
    return () => { window.clearInterval(interval); window.removeEventListener('focus', refreshClock); document.removeEventListener('visibilitychange', refreshClock) }
  }, [])
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    modal.querySelector('.modal-actions')?.before(fields)
  ...
  useEffect(() => {
    ...
    if (onBack) {
      ...
      back.type = 'button'; back.textContent = '← Volver a gestión'; back.onclick = onBack
      desktopNavigation.querySelector('nav').prepend(back)
    }
      ...
      if (view === 'history') {
        ...
        if (!blockingControl && !serviceRecord?.vehicleControl) {
          ...
          if (serviceUnlocked && serviceRecord.address) {
            ...
            directions.prepend(createIconElement('navigation'))
          ...
          if (serviceUnlocked && serviceRecord.phone) {
            ...
            call.prepend(createIconElement('phone'))
          ...
          const syncDetailState = expanded => {
            ...
            detailToggle.prepend(createIconElement(expanded ? 'collapse' : 'eye'))
          }
          ...
          start.prepend(createIconElement(serviceRecord.startedAt ? 'check' : 'play'))
  ...
  const saveStatus = async () => {
    ...
    const current = history.find(item => String(item.id) === String(record.id)) || record
    if (current.advanceRequest?.status === 'pending') throw new Error('El adelanto está pendiente de aprobación. Esperá la aprobación de Administración.')
    ...
    setHistory(previous => previous.map(item => item.id === record.id ? { ...item, ...updated } : item))
    setDraft(null); setObservation(''); vehicleMileageRef.current = ''; vehiclePhotoRef.current = ''
  }
  const requestStatus = (record, type) => {
    if (record.vehicleControl && !vehicleControlIsOpen(record, clock)) return
    if ((type === 'Cancelado' || type === 'Reprogramación solicitada') && !observation.trim()) return
    setConfirm({ record, type })
  }
  if (view === 'help') return <main className="technician-page"><header className="technician-header"><img src="/logo-pignus.png" alt="Pignus" /><div><b>{user.name}</b><span>{user.email}</span></div><button className="logout-button" onClick={() => setConfirm({ logout: true })}><Icon name="logout" size={17} />Cerrar sesión</button></header><section className="technician-content technician-help-page"><TechnicianHelpCenter onNavigate={setView} /></section>{confirm?.logout && <Confirm title="Cerrar sesión" detail="¿Querés cerrar sesión?" action={logout} confirmLabel="Sí, cerrar sesión" close={() => setConfirm(null)} />}</main>
  if (view === 'vehicles') return <main className="technician-page"><header className="technician-header"><img src="/logo-pignus.png" alt="Pignus" /><div><b>{user.name}</b><span>{user.email}</span></div><button className="logout-button" onClick={() => setConfirm({ logout: true })}><Icon name="logout" size={17} />Cerrar sesión</button></header><section className="technician-content"><p className="eyebrow">DOCUMENTACIÓN DE FLOTA</p><h1>Vehículos</h1><p className="technician-help">Consultá la flota y descargá el seguro vigente cuando lo necesites.</p><div className="data-card technician-vehicle-table"><div className="technician-vehicle-head"><span>Vehículo</span><span>Año</span><span>Matrícula</span><span>Seguro</span></div>{vehicles.length ? vehicles.map(vehicle => <div className="technician-vehicle-row" key={vehicle.id}><b>{vehicle.brand} {vehicle.model}</b><span>{vehicle.year}</span><strong>{vehicle.plate}</strong><span>{vehicle.insuranceFileName ? <a className="secondary" href={`/api/vehicle-insurance/${encodeURIComponent(String(vehicle.id))}`} download><Icon name="download" size={17} />Descargar PDF</a> : 'No disponible'}</span></div>) : <div className="empty-state">No hay vehículos registrados.</div>}</div></section>{confirm?.logout && <Confirm title="Cerrar sesión" detail="¿Querés cerrar sesión?" action={logout} confirmLabel="Sí, cerrar sesión" close={() => setConfirm(null)} />}</main>
  return <main className="technician-page"><header className="technician-header"><img src="/logo-pignus.png" alt="Pignus" /><div><b>{user.name}</b><span>{user.email}</span></div><button className="logout-button" onClick={() => setConfirm({ logout: true })}><Icon name="logout" size={17} />Cerrar sesión</button></header><section className="technician-content"><p className="eyebrow">MI AGENDA</p><h1>Servicios asignados</h1><p className="technician-help">Consultá los servicios pendientes de hoy y mañana.</p>{view === 'history' && <div className="technician-history-search"><label htmlFor="technician-history-query">Buscar en mi historial</label><div><Icon name="search" size={18} /><input id="technician-history-query" type="search" value={historySearch} onChange={event => setHistorySearch(event.target.value)} placeholder="Cliente, código, servicio, fecha o detalle..." autoComplete="off" /></div><small>{services.length} de {completedServices.length} servicio(s)</small></div>}{services.length ? services.map((record, index) => { const unlocked = record.vehicleControl || view === 'history' || index === 0 || resolved(services[index - 1]); const done = resolved(record); const earlyVehicleControl = record.vehicleControl && !vehicleControlIsOpen(record, clock); const canComplete = serviceHasStarted(record, clock) && (record.vehicleControl || Boolean(record.startedAt)); const technicianExtras = technicianVisibleServiceExtras(record); return <article className={`technician-service ${unlocked ? '' : 'locked'} ${earlyVehicleControl ? 'vehicle-control-early' : ''}`} key={record.id}><div className="technician-service-head"><span>{index + 1}</span><div><b>{record.time ? `${record.time} Hs` : 'Horario a confirmar'}</b><small>{prettyDate(record.date)}</small></div><em className={`work-status ${done ? 'completado' : 'pendiente'}`}>{record.technicalStatus || record.status || 'Pendiente'}</em></div><h2>{record.service}</h2>{journeyLabel(record, history) && <p className="journey-label">{journeyLabel(record, history)} · {journeyReportType(record, history) === 'Avance registrado' ? 'Al terminar, registrá el avance. El servicio continúa en otra jornada.' : 'Última jornada programada.'}</p>}<p className="tech-client">{record.client}</p>{view === 'agenda' && <div className="technician-crew-summary"><strong>{record.team || 'Equipo asignado'}</strong><span>{technicianCrewLabel(record, user.id)}</span></div>}{record.subscriberReservation && <p className="technician-reservation-notice"><Icon name="alert" size={16} />Reserva de nuevo abonado: el PIG definitivo todavía no fue vinculado.</p>}<p><b>Detalle:</b> {record.detail || 'Sin observaciones'}</p>{technicianExtras.paymentMethod && <p><b>Forma de pago:</b> {technicianExtras.paymentMethod}{technicianExtras.amount ? ` · Monto: ${formatCurrencyAmount(technicianExtras.amount)}` : ''}</p>}{technicianExtras.form && <p><b>Formulario:</b> {technicianExtras.form}</p>}{record.vehicleControl ? null : unlocked ? <><p><b>Dirección:</b> {record.address || 'Sin dirección'}</p><p><b>Contacto:</b> {record.phone || 'Sin contacto'}</p>{!record.subscriberReservation && <button className="secondary technician-customer-history" onClick={() => setCustomerHistoryRecord(record)}><Icon name="history" size={17} />Ver historial del cliente</button>}</> : <p className="locked-info">La dirección y el contacto se habilitarán al informar el estado del servicio anterior.</p>}{unlocked && !done && <>{earlyVehicleControl && <p className="vehicle-control-early-notice"><Icon name="lock" size={16} />Este control se habilita el {vehicleControlWindowLabel(record)}.</p>}{!canComplete && !earlyVehicleControl && <p className="vehicle-control-early-notice"><Icon name="lock" size={16} />{!record.vehicleControl && !record.startedAt ? 'Al llegar al domicilio, presioná Iniciar servicio. Para marcarlo completado primero debés iniciarlo.' : 'Podrás completarlo cuando llegue su horario programado.'}</p>}<div className="technician-actions">{record.advanceRequest?.status === "pending" && <p role="status">Adelanto pendiente: Administración debe aprobarlo para habilitar las acciones.</p>}<button className="primary" disabled={earlyVehicleControl || !canComplete} onClick={() => { setDraft({ record, type: journeyReportType(record, history) }); setObservation('') }}><Icon name={earlyVehicleControl || !canComplete ? 'lock' : 'check'} />{record.vehicleControl && earlyVehicleControl ? 'Todavía no disponible' : record.serviceJourney ? (journeyReportType(record, history) === 'Avance registrado' ? 'Registrar avance' : 'Completar servicio') : 'Marcar completado'}</button>{!record.vehicleControl && <><button className="secondary" disabled={record.advanceRequest?.status === "pending"} onClick={() => { setDraft({ record, type: 'Reprogramación solicitada' }); setObservation('') }}>Solicitar reprogramación</button><button className="secondary" disabled={record.advanceRequest?.status === "pending"} onClick={() => { setDraft({ record, type: 'Cancelado' }); setObservation('') }}>Informar cancelación</button></>}</div></>}{done && record.technicalReportedAt && <small className="reported-at">Informado: {prettyReportDateTime(record.technicalReportedAt)}</small>}</article> }) : <div className="empty-state">{view === 'history' && historySearch.trim() ? 'No hay servicios que coincidan con la búsqueda.' : view === 'history' ? 'Todavía no informaste servicios.' : 'No tenés servicios pendientes para hoy ni para mañana.'}</div>}</section>{draft && <div className="modal-layer"><div className="modal technician-status-modal"><button className="close-modal" onClick={() => setDraft(null)}><Icon name="close" /></button><p className="eyebrow">ACTUALIZAR SERVICIO</p><h2>{draft.type === 'Avance registrado' ? 'Registrar avance de la jornada' : draft.type}</h2><p>{draft.record.client} · {draft.record.service}</p>{draft.record.vehicleControl ? <p className="vehicle-control-report-help">Informá el kilometraje y adjuntá la foto del interior. No hace falta agregar una observación.</p> : <label><RequiredLabel>Observación</RequiredLabel><textarea required value={observation} onChange={event => setObservation(event.target.value)} placeholder="Detallá el trabajo realizado, el resultado y cualquier recomendación para futuras visitas." /></label>}<div className="modal-actions"><button className="secondary" onClick={() => setDraft(null)}><Icon name="close" size={16} />Cancelar</button><button className="primary" disabled={!draft.record.vehicleControl && !observation.trim()} onClick={() => setConfirm({ record: draft.record, type: draft.type })}>Continuar</button></div></div></div>}{customerHistoryRecord && <CustomerServiceHistory customer={customerHistoryRecord} history={history} close={() => setCustomerHistoryRecord(null)} technicianView />}{confirm?.record && <Confirm title="Confirmar estado" detail={`¿Confirmás que querés informar “${confirm.type}”? Luego quedará registrado y cualquier cambio deberá ser revisado por Administración.`} action={saveStatus} confirmLabel="Sí, confirmar estado" close={() => setConfirm(null)} />}{confirm?.logout && <Confirm title="Cerrar sesión" detail="¿Querés cerrar sesión?" action={logout} confirmLabel="Sí, cerrar sesión" close={() => setConfirm(null)} />}</main>
}
...
function HistoryReadOnly({ history, supervisorView = false, reminderFilter = null, clearReminderFilter = () => {} }) {
  ...
  const records = history.filter(record => normalizeSearchText(`${record.client} ${record.service} ${record.technicians?.join(' ')}`).includes(normalizeSearchText(search))).sort(supervisorView ? sortHistoryByDateAndTime : sortOperationalHistory)
  return <><div className="module-intro"><div><p className="eyebrow">{supervisorView ? 'SEGUIMIENTO DE CUENTAS' : 'TRABAJOS REALIZADOS'}</p><h1>{supervisorView ? 'Seguimiento de CCTV' : 'Historial técnico'}</h1><p>{supervisorView ? 'Consulta de solo lectura de los abonados y clientes marcados con Servicio de CCTV.' : 'Consulta de solo lectura. Los cambios de estado y datos requieren el permiso Gestionar historial.'}</p></div>{(reminderFilter || search) && <button className="secondary" onClick={() => { setSearch(''); clearReminderFilter() }}><Icon name="close" size={16} />Limpiar filtros</button>}</div><div className="accounts-bar history-toolbar"><div><b>{history.length}</b> trabajos registrados</div><label><Icon name="search" size={16} /><input placeholder="Buscar cliente, servicio o técnico..." value={search} onChange={event => setSearch(event.target.value)} /></label></div><div className="data-card history-table"><div className="table-head"><span>Fecha</span><span>Cliente</span><span>Servicio</span><span>Estado</span><span>Detalle</span></div>{records.length ? records.map(record => { const recordStatus = historyStatusLabel(record); return <div className="history-row" key={record.id}><b>{prettyDate(record.date)}</b><div className="history-client"><strong>{record.client}</strong>{record.awaitingConfirmation === true && <span className="service-confirmation-label">(A CONFIRMAR)</span>}<small>{record.address || 'Sin dirección'}</small></div><div><em className="role-chip">{record.service}</em></div><div><span className={`work-status ${recordStatus.toLowerCase().replace(/\s/g, '-')}`}>{recordStatus}</span></div><div><button className="secondary detail-button" onClick={() => setDetail(record)}><Icon name="eye" size={16} />Ver detalle</button></div></div> }) : <div className="empty-state">{supervisorView ? 'Todavía no hay servicios de cuentas con seguimiento de CCTV.' : 'No hay trabajos para mostrar.'}</div>}</div>{detail && <HistoryDetail record={detail} hideInternal={supervisorView} close={() => setDetail(null)} />}</>
}
...
function HistoryBulkView({ history, setHistory, customers, services, employees, authUser, persistHistoryRecord, persistHistoryRecordRemoval, historyScheduling, reminderFilter = null, clearReminderFilter = () => {} }) {
  ...
  const hasActiveFilters = Boolean(reminderFilter || search || fromDate || toDate || statusFilter !== 'all' || serviceFilter)
  const clearFilters = () => {
    setSearch(''); setFromDate(''); setToDate(''); setStatusFilter('all'); setServiceFilter('')
    setHistoryPage(1); setSelected([]); clearReminderFilter()
    document.querySelectorAll('.history-date-filters input').forEach(input => { input.value = '' })
    const selects = document.querySelectorAll('.history-date-filters select')
    if (selects[0]) selects[0].value = 'all'
  ...
  const status = record => historyStatusLabel(record)
  ...
  useEffect(() => {
    ...
    if (serviceRecordFingerprint(detail) !== serviceRecordFingerprint(liveDetail)) setDetail(liveDetail)
  }, [detail, liveDetail])
  ...
  useEffect(() => {
    ...
    ;[['all', 'Todos'], ['Pendiente', 'Pendiente'], ['Avance registrado', 'Avance registrado'], ['Completado', 'Completado'], ['Requiere revisión', 'Requiere revisión'], ['Reprogramado', 'Reprogramado'], ['Cancelado', 'Cancelado']].forEach(([value, label]) => {
    ...
    filters.append(from.field, to.field, statusField, serviceField)
  ...
  return <><div className="module-intro"><div><p className="eyebrow">TRABAJOS REALIZADOS</p><h1>Historial técnico</h1><p>Seleccioná varios servicios para confirmarlos, cancelarlos o reprogramarlos en una sola acción.</p></div><div className="history-header-actions">{hasActiveFilters && <button className="secondary" onClick={clearFilters}><Icon name="close" size={16} />Limpiar filtros</button>}<button className="primary" disabled={!selected.length} onClick={() => setBulkOpen(true)}><Icon name="check" />{selected.length ? `Gestionar ${selected.length} seleccionados` : 'Gestionar selección'}</button></div></div><div className="accounts-bar history-toolbar"><div><b>{history.length}</b> trabajos registrados</div><label><Icon name="search" size={16} /><input placeholder="Buscar cliente, servicio o técnico..." value={search} onChange={event => setSearch(event.target.value)} /></label></div><div className="data-card history-table history-bulk"><div className="table-head"><span><input aria-label="Seleccionar todos" type="checkbox" checked={records.length > 0 && selected.length === records.length} onChange={toggleAll} /></span><span>Fecha</span><span>Hora</span><span>Cliente</span><span>Servicio</span><span>Técnicos asignados</span><span>Estado</span><span>Acciones</span></div>{records.length ? records.map(record => <div className="history-row" key={record.id}><span><input aria-label={`Seleccionar ${record.client}`} type="checkbox" checked={selected.includes(record.id)} onChange={() => toggle(record.id)} /></span><b>{prettyDate(record.date)}</b><div className="history-time" aria-label={`Hora asignada: ${record.time || record.scheduledTime ? `${record.time || record.scheduledTime} Hs` : 'A confirmar'}`}>{record.time || record.scheduledTime ? `${record.time || record.scheduledTime} Hs` : 'A confirmar'}</div><div className="history-client"><strong>{record.client}</strong>{record.awaitingConfirmation === true && <span className="service-confirmation-label">(A CONFIRMAR)</span>}<small>{record.address || 'Sin dirección'}</small></div><div><em className="role-chip">{record.service}</em></div><div className="history-technicians" title={record.technicians?.join(' / ') || 'Sin asignar'}><span>{technicianNames(record)}</span></div><div><span className={`work-status ${status(record).toLowerCase().replace(/\s/g, '-')}`}>{status(record)}</span>{record.scheduledDate && <small className="scheduled-date">Para: {prettyDate(record.scheduledDate)}</small>}</div><div><button className="secondary detail-button" onClick={() => setDetail(record)}><Icon name="eye" size={16} />Gestionar</button></div></div>) : <div className="empty-state">No hay trabajos para mostrar.</div>}</div>{bulkOpen && <div className="modal-layer"><div className="modal bulk-modal"><button className="close-modal" onClick={() => setBulkOpen(false)}><Icon name="close" /></button><p className="eyebrow">GESTIÓN MÚLTIPLE</p><h2>{selected.length} servicio(s) seleccionados</h2><p>La modificación se aplicará a todos los servicios elegidos.</p><label>Nuevo estado<select value={bulkStatus} onChange={event => setBulkStatus(event.target.value)}><option>Completado</option><option>Cancelado</option><option>Reprogramado</option></select></label>{bulkStatus === 'Reprogramado' && <label>Reprogramar para<input type="date" min={minimumRescheduleDate} value={rescheduleDate} onChange={event => setRescheduleDate(event.target.value)} /></label>}<div className="modal-actions"><button className="secondary" onClick={() => setBulkOpen(false)}><Icon name="close" size={16} />Cancelar</button><button className="primary" disabled={bulkStatus === 'Reprogramado' && (!rescheduleDate || rescheduleDate < minimumRescheduleDate)} onClick={applyBulk}><Icon name="check" size={16} />Aplicar cambios</button></div></div></div>}{detail && <HistoryManagementDetail record={detail} setHistory={setHistory} close={() => setDetail(null)} customers={customers} services={services} employees={employees} authUser={authUser} persistHistoryRecord={persistHistoryRecord} persistHistoryRecordRemoval={persistHistoryRecordRemoval} historyScheduling={historyScheduling} />}</>
}

function HistoryManagement({ history, setHistory, customers, services, employees, authUser, persistHistoryRecord, persistHistoryRecordRemoval, historyScheduling, reminderFilter = null, clearReminderFilter = () => {} }) {
  return <HistoryBulkView {...{ history, setHistory, customers, services, employees, authUser, persistHistoryRecord, persistHistoryRecordRemoval, historyScheduling, reminderFilter, clearReminderFilter }} />
  const [search, setSearch] = useState('')
  const [detail, setDetail] = useState(null)
  const records = history.filter(record => normalizeSearchText(`${record.client} ${record.service} ${record.technicians?.join(' ')}`).includes(normalizeSearchText(search))).sort(sortHistoryByDateAndTime)
  const status = record => historyStatusLabel(record)
  return <><div className="module-intro"><div><p className="eyebrow">TRABAJOS REALIZADOS</p><h1>Historial técnico</h1><p>Gestioná la confirmación, cancelación o reprogramación de cada servicio.</p></div></div><div className="accounts-bar history-toolbar"><div><b>{history.length}</b> trabajos registrados</div><label><Icon name="search" size={16} /><input placeholder="Buscar cliente, servicio o técnico..." value={search} onChange={event => setSearch(event.target.value)} /></label></div><div className="data-card history-table"><div className="table-head"><span>Fecha</span><span>Cliente</span><span>Servicio</span><span>Estado</span><span>Detalle</span></div>{records.length ? records.map(record => <div className="history-row" key={record.id}><b>{prettyDate(record.date)}</b><div className="history-client"><strong>{record.client}</strong>{record.awaitingConfirmation === true && <span className="service-confirmation-label">(A CONFIRMAR)</span>}<small>{record.address || 'Sin dirección'}</small></div><div><em className="role-chip">{record.service}</em></div><div><span className={`work-status ${status(record).toLowerCase().replace(/\s/g, '-')}`}>{status(record)}</span>{record.scheduledDate && <small className="scheduled-date">Para: {prettyDate(record.scheduledDate)}</small>}</div><div><button className="secondary detail-button" onClick={() => setDetail(record)}><Icon name="eye" size={16} />Gestionar</button></div></div>) : <div className="empty-state">No hay trabajos para mostrar.</div>}</div>{detail && <HistoryManagementDetail record={detail} setHistory={setHistory} close={() => setDetail(null)} customers={customers} services={services} employees={employees} authUser={authUser} />}</>
}

function HistoryManagementDetail({ record, setHistory, close, customers, services, employees, authUser, persistHistoryRecord, persistHistoryRecordRemoval, historyScheduling }) {
  const journeyHistory = React.useContext(ServiceJourneysContext)?.history || [record]
  const [rescheduleDate, setRescheduleDate] = useState('')
  const [rescheduling, setRescheduling] = useState(false)
  const [rescheduleTeam, setRescheduleTeam] = useState('')
  const [rescheduleTime, setRescheduleTime] = useState('')
  const [rescheduleError, setRescheduleError] = useState('')
  const [reschedulePrompt, setReschedulePrompt] = useState(null)
  const rescheduleSlotComplete = completeRescheduleSlot(rescheduleDate, rescheduleTime)
  const destinationTeams = rescheduleSlotComplete ? historyScheduling?.teamsForDate(rescheduleDate, rescheduleTime, record) || [] : []
  const confirmReschedule = async () => {
    if (interactionBlocked || !rescheduleReady) return
    setSaving(true); setRescheduleError('')
    try {
      await historyScheduling.save(record, rescheduleDate, rescheduleTeam, rescheduleTime)
      close()
    } catch (error) { setRescheduleError(error.message || 'No se pudo reprogramar. Intentá nuevamente.') }
    finally { setSaving(false) }
  }
  const minimumRescheduleDate = new Date().toLocaleDateString('sv-SE', { timeZone: 'America/Argentina/Buenos_Aires' })
  ...
  const rescheduleReady = Boolean(rescheduleDate >= minimumRescheduleDate && (!requiresDifferentRescheduleDay(record) || rescheduleDate !== record.date) && selectedRescheduleTeam && /^([01]\d|2[0-3]):[0-5]\d$/.test(rescheduleTime))
  const requestClose = () => {
    if (saving) return
  ...
  const assignedDayTeams = historyScheduling?.assignedTeamsForDate?.(record.date) || []
  const assignedDayTeam = assignedDayTeams.find(team => String(team.teamId) === String(record.teamId))
  const assignedTeamLabel = assignedDayTeam ? `${assignedDayTeam.label} · ${assignedDayTeam.members.join(' / ')}` : `${record.team || 'Sin equipo'} · ${record.technicians?.join(' / ') || 'Sin técnicos asignados'}`
  const editableDraft = value => ({ estimatedMinutes: serviceEstimateForTask(value, resolveServiceForTask(value, services)), estimatedMinutesCustomized: Boolean(value.estimatedMinutesCustomized), customerId: value.customerId || '', serviceId: value.serviceId || '', technicianIds: value.technicianIds || [], teamId: value.teamId || '', team: value.team || '', address: value.address || '', phone: value.phone || '', detail: value.detail || '', internalNote: value.internalNote || '', internalChecklist: normalizeInternalChecklist(value.internalChecklist) })
  ...
  const update = async patch => {
    ...
    if (isReschedule) {
      window.dispatchEvent(new CustomEvent('pignus:reschedule-service', { detail: { record: tracedRecord, nextDate: patch.scheduledDate, sourceDate: record.date } }))
      setHistory(previous => previous.map(item => item.id === record.id ? tracedRecord : item)); close(); return
    }
    if (!persistHistoryRecord) { setHistory(previous => previous.map(item => item.id === record.id ? tracedRecord : item)); close(); return }
    setSaving(true)
    try { await persistHistoryRecord(record, tracedRecord); close() } finally { setSaving(false) }
  }
  useEffect(() => {
    if (editing || record.vehicleControl) return undefined
    const heading = document.querySelector('.history-detail .history-detail-heading')
    if (!heading || heading.querySelector('.history-photo-actions')) return undefined
    const actions = document.createElement('div'); actions.className = 'history-photo-actions'
    const gallery = document.createElement('input'); gallery.type = 'file'; gallery.accept = 'image/*'; gallery.hidden = true
    const camera = document.createElement('input'); camera.type = 'file'; camera.accept = 'image/*'; camera.setAttribute('capture', 'environment'); camera.hidden = true
    const galleryButton = document.createElement('button'); galleryButton.type = 'button'; galleryButton.className = 'secondary small'; galleryButton.textContent = record.servicePhotoAttached ? 'Cambiar foto' : 'Adjuntar foto'; galleryButton.onclick = () => gallery.click()
    const cameraButton = document.createElement('button'); cameraButton.type = 'button'; cameraButton.className = 'secondary small'; cameraButton.textContent = 'Sacar foto'; cameraButton.onclick = () => camera.click()
    const status = document.createElement('small')
    const upload = async event => {
      const file = event.target.files?.[0]
      if (!file) return
      galleryButton.disabled = true; cameraButton.disabled = true; status.textContent = 'Preparando foto…'
      try {
        const photo = await compactVehiclePhoto(file)
        const result = await servicePhotoRepository.upload(record.id, photo)
        await update({ servicePhotoAttached: true, servicePhotoUrl: result.url, servicePhotoAttachedAt: result.createdAt })
      } catch (photoError) {
        status.textContent = photoError.message; status.className = 'field-error'; galleryButton.disabled = false; cameraButton.disabled = false
      }
      event.target.value = ''
    }
    gallery.onchange = upload; camera.onchange = upload
    galleryButton.prepend(createIconElement('upload')); cameraButton.prepend(createIconElement('camera'))
    actions.append(galleryButton, cameraButton)
    if (record.servicePhotoAttached) {
      const removeButton = document.createElement('button'); removeButton.type = 'button'; removeButton.className = 'danger-button small'; removeButton.textContent = 'Quitar foto'
      removeButton.onclick = async () => { try { await servicePhotoRepository.remove(record.id); await update({ servicePhotoAttached: false, servicePhotoUrl: '', servicePhotoAttachedAt: '' }) } catch (photoError) { status.textContent = photoError.message; status.className = 'field-error' } }
      removeButton.prepend(createIconElement('trash'))
      actions.append(removeButton)
    }
    actions.append(gallery, camera, status); heading.append(actions)
    return () => actions.remove()
  }, [editing, record.id, record.servicePhotoAttached, saving])
  const remove = async () => {
    if (interactionBlocked) return
    if (!persistHistoryRecordRemoval) { setHistory(previous => previous.filter(item => item.id !== record.id)); close(); return }
    setSaving(true)
    try { await persistHistoryRecordRemoval(record); close() } finally { setSaving(false) }
  }
  const saveChanges = () => {
    if (interactionBlocked) return
    ...
    if ((draft.customerId !== (record.customerId || '') && !customer) || (draft.serviceId !== (record.serviceId || '') && !service)) return
    const patch = { ...draft, ...(record.vehicleControl ? { estimatedMinutes: 15, estimatedMinutesCustomized: false } : {}), ...(customer ? customerLinkPatch(record, customer, authUser) : {}), ...(service ? { serviceId: service.id, service: service.name } : {}), address: customer ? customer.address || '' : draft.address, phone: customer ? customer.phone || '' : draft.phone, teamId: record.teamId || '', team: record.team || '', technicianIds: record.technicianIds || [], technicians: record.technicians || [] }
    if (!persistHistoryRecord) window.dispatchEvent(new CustomEvent('pignus:sync-agenda-service', { detail: { record, patch: patch } }))
    update(patch)
  }
  const status = record.status || 'Pendiente'
  const setField = field => event => {
    const value = event.target.value
    const customer = field === 'customerId' ? customers.find(item => String(item.customerId) === String(value)) : null
    setDraft(previous => ({ ...previous, [field]: value, ...(customer ? { address: customer.address || '', phone: customer.phone || '' } : {}) }))
  }
  useEffect(() => {
  ...
  useEffect(() => {
    if (editing || record.vehicleControl || !servicePhotoUrl(record)) return undefined
    const grid = document.querySelector('.history-detail .history-detail-grid')
    if (!grid || grid.querySelector('.history-service-photo-detail')) return undefined
    const detail = document.createElement('div'); detail.className = 'detail-notes history-service-photo-detail'
    const title = document.createElement('b'); title.textContent = 'Foto de referencia del servicio'
    const link = document.createElement('a'); link.className = 'service-photo history-service-photo'; link.href = servicePhotoUrl(record); link.target = '_blank'; link.rel = 'noreferrer'; link.title = 'Abrir foto en tamaño completo'
    const photo = document.createElement('img'); photo.src = servicePhotoUrl(record); photo.alt = `Foto de referencia de ${record.client || 'servicio'}`; photo.onerror = () => detail.remove()
    link.append(photo); detail.append(title, link); grid.append(detail)
    return () => detail.remove()
  }, [record, editing])
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    const intercept = event => {
      ...
      let action = null
      if (text.includes('registrar avance')) action = { title: 'Registrar avance de la jornada', detail: 'Se cerrará esta visita, pero el servicio seguirá abierto hasta la última jornada.', confirmLabel: 'Registrar avance', icon: '✓', patch: { status: 'Avance registrado', scheduledDate: '' } }
  ...
  return <><div className="modal-layer"><div className="modal detail-modal history-detail"><button className="close-modal" disabled={saving} onClick={requestClose}><Icon name="close" /></button><p className="eyebrow">{prettyDate(record.date)} · {historyStatusLabel(record).toUpperCase()}</p><div className="history-detail-heading"><h2>{editing ? 'Editar servicio' : record.client}</h2><button className="secondary detail-edit" disabled={interactionBlocked} onClick={() => setEditing(!editing)}><Icon name="edit" size={15} />{editing ? 'Cancelar edición' : 'Editar datos'}</button></div>{!editing && record.awaitingConfirmation === true && <div className="history-confirmation-actions"><p className="service-confirmation-label">(A CONFIRMAR)</p>{record.date < currentLocalDate() ? <p>El día del servicio ya pasó. Reprogramalo antes de confirmarlo.</p> : null}</div>}{record.subscriberReservation && <p className="history-reservation-notice"><Icon name="alert" size={17} />Esta es una reserva de nuevo abonado. Presioná Editar datos y vinculá el PIG importado; el turno, equipo y duración se conservarán.</p>}{record.reservationLinkedAt && <p className="history-reservation-linked"><Icon name="check" size={17} />Reserva vinculada el {prettyReportDateTime(record.reservationLinkedAt)}. Datos provisorios originales: {record.reservationOriginal?.name || 'sin nombre'} · {record.reservationOriginal?.address || 'sin dirección'} · {record.reservationOriginal?.phone || 'sin contacto'}.</p>}{editing ? <div className="history-edit-grid"><JourneyIdentityField record={record} customer><CustomerAutocomplete value={draft.client || record.client} customerId={draft.customerId} customers={customers} subscriberReservation={record.subscriberReservation} onCustomerSelect={customer => setDraft(previous => ({ ...previous, customerId: customer.customerId, client: `${customer.account} ${customer.name}`, address: customer.address || '', phone: customer.phone || '' }))} /></JourneyIdentityField><JourneyIdentityField record={record}><label>Tipo de servicio{<select value={draft.serviceId} onChange={setField('serviceId')}><option value="">Seleccionar</option>{services.map(service => <option key={service.id} value={service.id}>{service.name}</option>)}</select>}</label></JourneyIdentityField>{record.vehicleControl ? <label>Tiempo estimado<input value="15 minutos" readOnly /></label> : <ServiceEstimatedDurationField value={draft.estimatedMinutes} onChange={estimatedMinutes => setDraft(previous => ({ ...previous, estimatedMinutes, estimatedMinutesCustomized: true }))} />}<label>Equipo y técnicos<select disabled value={record.teamId || ''} aria-label="Equipo y técnicos asignados"><option value={record.teamId || ''}>{assignedTeamLabel}</option>{assignedDayTeams.filter(team => String(team.teamId) !== String(record.teamId)).map(team => <option key={team.teamId} value={team.teamId}>{team.label} · {team.members.join(' / ')}</option>)}</select><small>La asignación se conserva. Para elegir otro día, horario y equipo, utilizá Reprogramar.</small></label><label>Dirección<input readOnly={Boolean(draft.customerId)} value={draft.address} onChange={setField('address')} /></label><label>Contacto<input readOnly={Boolean(draft.customerId)} value={draft.phone} onChange={setField('phone')} /></label><label className="detail-notes">Detalle / observaciones<textarea value={draft.detail} onChange={setField('detail')} /></label></div> : <div className="history-detail-grid"><div><b>Servicio</b><span>{record.service}</span></div><div><b>Tiempo estimado</b><span>{durationLabel(serviceEstimateForTask(record, resolveServiceForTask(record, services)))}</span></div><div><b>Equipo</b><span>{record.team}</span></div><div><b>Técnicos asignados</b><span>{record.technicians?.join(' / ') || 'Sin técnicos asignados'}</span></div><div><b>Hora asignada</b><span>{record.time || record.scheduledTime || 'Sin horario'}</span></div>{record.completedAt && <div><b>Finalización real</b><span>{prettyReportDateTime(record.completedAt)}</span></div>}<div><b>Dirección</b><span>{record.address || 'Sin dirección'}</span></div><div><b>Contacto</b><span>{record.phone || 'Sin contacto'}</span></div>{record.formEmail && <div><b>Email del formulario</b><span>{record.formEmail}</span></div>}<div className="detail-notes"><b>Detalle / observaciones</b><span>{record.detail || 'Sin observaciones'}</span></div></div>}{!editing && <JourneyHistory record={record} />}<InternalPreparationFields task={editing ? draft : record} readOnly={!editing} onChange={patch => setDraft(previous => ({ ...previous, ...patch }))} />{editing ? <div className="history-actions"><button className="primary" disabled={interactionBlocked} onClick={saveChanges}><Icon name="check" />Guardar cambios</button><button className="secondary" disabled={interactionBlocked} onClick={() => setEditing(false)}><Icon name="close" size={16} />Cancelar</button></div> : <div className="history-actions">{record.awaitingConfirmation === true && <ServiceConfirmationButton task={{ ...record, historyId: record.id }} day={record.date} history={[record]} disabled={interactionBlocked} persist={command => update({ awaitingConfirmation: command.awaitingConfirmation })} />}{(status === 'Pendiente' || (status === 'Avance registrado' && journeyReportType(record, journeyHistory) === 'Completado')) ? <button className="primary" disabled={interactionBlocked} onClick={() => update({ status: journeyReportType(record, journeyHistory), scheduledDate: '', ...(journeyReportType(record, journeyHistory) === 'Completado' ? { completedAt: new Date().toISOString() } : {}) })}><Icon name="check" />{journeyReportType(record, journeyHistory) === 'Avance registrado' ? 'Registrar avance' : 'Marcar completado'}</button> : <button className="pending-button" disabled={interactionBlocked} onClick={() => update({ status: 'Pendiente', scheduledDate: '', technicalStatus: '', technicalObservation: '', technicalReportedAt: '', completedAt: '' })}><Icon name="history" />Marcar pendiente</button>}<button className="secondary" disabled={interactionBlocked} onClick={() => update({ status: 'Cancelado', scheduledDate: '' })}><Icon name="close" />Cancelar servicio</button><button className="secondary reschedule-trigger" aria-expanded={rescheduling} disabled={interactionBlocked || (rescheduling && Boolean(rescheduleDate) && !rescheduleReady)} onClick={() => { if (!rescheduling) setRescheduling(true); else if (rescheduleReady) { setRescheduleError(''); setReschedulePrompt('confirm') } }}><Icon name="calendar" />{saving && rescheduling ? 'Guardando…' : rescheduling && rescheduleDate ? 'Confirmar reprogramación' : 'Reprogramar'}</button><button className="danger-button delete-history" disabled={interactionBlocked || Boolean(record.serviceJourney)} title={record.serviceJourney ? "Las jornadas vinculadas se cancelan para conservar el historial." : undefined} onClick={() => setConfirmDelete(true)}><Icon name="trash" />Eliminar registro</button></div>}{!editing && rescheduling && <div className="history-reschedule-fields"><label>Reprogramar para<input type="date" min={minimumRescheduleDate} disabled={interactionBlocked} value={rescheduleDate} onChange={event => { setRescheduleDate(event.target.value); setRescheduleTeam(''); setRescheduleError('') }} /></label><label>Hora<input type="time" disabled={interactionBlocked} value={rescheduleTime} onChange={event => { setRescheduleTime(event.target.value); setRescheduleTeam(''); setRescheduleError('') }} /></label><label>Equipo<select disabled={interactionBlocked || !rescheduleSlotComplete || !destinationTeams.length} value={selectedRescheduleTeam ? rescheduleTeam : ''} onChange={event => setRescheduleTeam(event.target.value)}><option value="">{rescheduleSlotComplete ? 'Seleccionar equipo disponible' : 'Completá día y hora'}</option>{destinationTeams.map(team => <option key={team.teamId} value={team.teamId}>{team.label} · {team.members.join(' / ')}</option>)}</select></label>{rescheduleDate === record.date && requiresDifferentRescheduleDay(record) && <p className="field-error">La reprogramación pendiente requiere una fecha distinta del día original.</p>}{rescheduleSlotComplete && !destinationTeams.length && <p className="field-error">No hay equipos disponibles para ese día, horario y duración del servicio. Elegí otro día u horario.</p>}{rescheduleError && <p className="field-error" role="alert">{rescheduleError}</p>}</div>}</div></div>{reschedulePrompt && <div className="modal-layer history-reschedule-confirmation"><section className="modal confirm-modal" role="dialog" aria-modal="true" aria-label={reschedulePrompt === 'leave' ? 'Reprogramación sin guardar' : 'Confirmar reprogramación'}><button className="close-modal" disabled={saving} onClick={() => setReschedulePrompt(null)} aria-label="Seguir editando"><Icon name="close" /></button><h2>{reschedulePrompt === 'leave' ? 'Reprogramación sin guardar' : 'Confirmar reprogramación'}</h2><p>{reschedulePrompt === 'leave' ? 'Cargaste datos de reprogramación que todavía no se guardaron. Podés confirmarlos, seguir editando o cancelar esta reprogramación y cerrar.' : 'Revisá el destino antes de confirmar el cambio del servicio.'}</p><p><b>{record.client}</b><br />Día: {rescheduleDate ? prettyDate(rescheduleDate) : 'Sin seleccionar'}<br />Hora: {rescheduleTime || 'Sin seleccionar'}<br />Equipo: {selectedRescheduleTeam ? `${selectedRescheduleTeam.label} · ${selectedRescheduleTeam.members.join(' / ')}` : 'Sin seleccionar'}</p>{!rescheduleReady && <p>Completá una fecha válida, hora y equipo para poder confirmar.</p>}{remotelyUpdated && <p role="alert">El servicio cambió desde otra sesión. Volvé y revisá su versión actual.</p>}{rescheduleError && <p className="field-error" role="alert">{rescheduleError}</p>}<div className="confirm-actions"><button className="secondary" disabled={saving} onClick={() => setReschedulePrompt(null)}><Icon name="edit" size={16} />Seguir editando</button>{reschedulePrompt === 'leave' && <button className="danger-button" disabled={saving} onClick={close}><Icon name="close" size={16} />Cancelar reprogramación</button>}<button className="primary" disabled={interactionBlocked || !rescheduleReady} onClick={confirmReschedule}><Icon name="calendar" size={16} />{saving ? 'Guardando…' : 'Confirmar reprogramación'}</button></div></section></div>}{confirmDelete && <Confirm title="Eliminar registro" detail="¿Querés eliminar este servicio del historial? Esta acción no se puede deshacer." destructive action={remove} close={() => setConfirmDelete(false)} />}</> }

function HistoryDetail({ record, close, hideInternal = false }) {
  const hasTechnicalReport = Boolean(record.technicalReportedAt || record.technicalStatus || record.technicalObservation)
  return <div className="modal-layer"><div className="modal detail-modal history-detail"><button className="close-modal" onClick={close}><Icon name="close" /></button><p className="eyebrow">{prettyDate(record.date)} · {historyStatusLabel(record).toUpperCase()}</p><h2>{record.client}</h2><div className="history-detail-grid"><div><b>Servicio</b><span>{record.service}</span></div><div><b>Hora asignada</b><span>{record.time || record.scheduledTime ? `${record.time || record.scheduledTime} Hs` : 'Sin horario'}</span></div><div><b>Equipo</b><span>{record.team}</span></div><div><b>Técnicos asignados</b><span>{record.technicians?.join(' / ') || 'Sin técnicos asignados'}</span></div><div><b>Dirección</b><span>{record.address || 'Sin dirección'}</span></div><div><b>Contacto</b><span>{record.phone || 'Sin contacto'}</span></div>{record.formEmail && <div><b>Email del formulario</b><span>{record.formEmail}</span></div>}<div className="detail-notes"><b>Detalle / observaciones de agenda</b><span>{record.detail || 'Sin observaciones'}</span></div>{hasTechnicalReport && <div className="detail-notes customer-history-technical"><b>{technicalReporter(record)} indicó{record.technicalStatus ? ` · ${record.technicalStatus}` : ''}</b><span>{record.technicalObservation || 'Servicio informado sin observaciones adicionales.'}</span>{record.technicalReportedAt && <small>{prettyReportDateTime(record.technicalReportedAt)}</small>}</div>}{!record.vehicleControl && servicePhotoUrl(record) && <div className="detail-notes history-service-photo-detail"><b>Foto de referencia del servicio</b><ServicePhoto record={record} className="history-service-photo" /></div>}</div><JourneyHistory record={record} />{!hideInternal && <InternalPreparationFields task={record} readOnly onChange={() => {}} />}</div></div>
}

function Accounts({ customers, setCustomers, setNotice, ask, history, teams, weekly, permissions = {}, isAdministrator = false, stateRevision, refreshRemoteState, applyCustomerImportState }) {
  ...
  const canEdit = isAdministrator || Boolean(permissions.accountsEdit)
  const canDelete = isAdministrator || Boolean(permissions.accountsDelete)
  const canImport = isAdministrator || Boolean(permissions.accountsImport)
  ...
  const undoImport = () => ask('Deshacer última importación', '¿Querés restaurar exactamente los abonados y clientes existentes antes de la última importación? Esta acción no afecta servicios ni agendas.', async () => {
    ...
    setNotice('La última importación fue deshecha y se restauró la base anterior.')
  }, true)
  const save = e => {
    e.preventDefault()
    const kind = customerKind(form)
    const customer = { ...form, customerId: form.customerId || createCustomerId(), kind, cctvService: Boolean(form.cctvService), account: normalizeAccountKey(form.account || nextCustomerCode(customers, kind)), name: normalizeCustomerName(form.name), address: [form.street, form.locality].filter(Boolean).join(', ') }
...
function Vehicles({ vehicles, setVehicles, setNotice, ask, isAdministrator, stateRevision, refreshRemoteState }) {
  ...
  const save = async event => {
    ...
    try {
      if (insuranceFile) {
        const response = await fetchWithTimeout(`/api/vehicle-insurance/${encodeURIComponent(String(record.id))}`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ fileName: insuranceFile.name, pdf: await readFileAsDataUrl(insuranceFile), insuranceExpiresOn: record.insuranceExpiresOn, vehicle: record, revision: stateRevision }) }, 60000)
        const payload = await response.json().catch(() => ({}))
        if (!response.ok) throw new Error(payload.error || 'No se pudo cargar el seguro.')
        record = payload.vehicle || { ...record, insuranceFileName: payload.fileName, insuranceUploadedAt: payload.uploadedAt, insuranceDocumentUrl: payload.documentUrl }
  ...
  return <>
    ...
    {open && <EditorModal active={Boolean(editing)} title="Editar vehículo" onClose={() => setOpen(false)} busy={saving}><form className="vehicle-form" onSubmit={save}>
      ...
      <button className="primary" disabled={saving}><Icon name="check" />{saving ? 'Guardando…' : editing ? 'Guardar cambios' : 'Guardar vehículo'}</button><button type="button" className="secondary" onClick={() => setOpen(false)}><Icon name="close" size={16} />Cancelar</button>
    </form></EditorModal>}
...
function Employees({ employees, setEmployees, roles, setNotice, ask, history, teams, weekly, canEnableTechnical = false }) {
  ...
  return <><div className="module-intro"><div><p className="eyebrow">EQUIPO PIGNUS</p><h1>Técnicos y colaboradores</h1><p>Administrá accesos, datos de contacto y disponibilidad del equipo.</p></div><button className="primary" onClick={() => { setForm(blankEmployee); setEditing(null); setOpen(true) }}><Icon name="plus" />Nuevo empleado</button></div>{open && <EmployeeForm {...{ form, setForm, roles, save, cancel: () => setOpen(false), editing, canEnableTechnical }} />}<div className="data-card employees-table"><div className="table-head">{['Empleado', 'Rol', 'Correo', 'Contacto', 'Estado', 'Acciones'].map(x => <span key={x}>{x}</span>)}</div>{employees.map(x => <div className="employee-row" key={x.id}><div className="person"><span>{initials(x.name)}</span><b>{x.name}</b></div><div><em className="role-chip">{roles.find(role => String(role.id) === String(x.roleId))?.name || x.role}</em>{x.technicalEnabled === true && <small>También realiza servicios técnicos</small>}</div><div>{x.email}</div><div>{x.phone || 'Sin teléfono'}</div><div><button className={`status ${x.status === 'Activo' ? 'on' : ''}`} onClick={() => ask('Cambiar estado', `¿Querés marcar a ${x.name} como ${x.status === 'Activo' ? 'inactivo' : 'activo'}?`, () => setEmployees(prev => prev.map(y => y.id === x.id ? { ...y, status: y.status === 'Activo' ? 'Inactivo' : 'Activo' } : y)))}>{x.status}</button></div><div className="row-actions"><button title="Editar empleado" onClick={() => { setForm(x); setEditing(x.id); setOpen(true) }}><Icon name="edit" size={16} /></button><button className="delete" title="Eliminar empleado" onClick={() => ask('Eliminar empleado', `¿Querés eliminar el perfil de ${x.name}?`, () => removeEmployee(x), true)}><Icon name="trash" size={16} /></button></div></div>)}</div></>
}
function EmployeeForm({ form, setForm, roles, save, cancel, editing, canEnableTechnical = false }) {
  const set = key => event => setForm({ ...form, [key]: event.target.value })
  return <EditorModal active={Boolean(editing)} title="Editar empleado" onClose={cancel}>
    <form className="employee-form employee-form-wide" onSubmit={save}>
      <label><RequiredLabel>Nombre</RequiredLabel><input required autoComplete="given-name" value={form.firstName || ''} onChange={set('firstName')} /></label>
      <label><RequiredLabel>Apellido</RequiredLabel><input required autoComplete="family-name" value={form.lastName || ''} onChange={set('lastName')} /></label>
      <label>Teléfono<input type="tel" autoComplete="tel" value={form.phone || ''} onChange={set('phone')} /></label>
      <label><RequiredLabel>Correo electrónico</RequiredLabel><input required type="email" autoComplete="email" value={form.email || ''} onChange={set('email')} /></label>
      <label><RequiredLabel>Rol</RequiredLabel><select required value={form.roleId ?? roles.find(role => role.name === form.role)?.id ?? ''} onChange={event => { const selectedRole = roles.find(role => String(role.id) === event.target.value); setForm({ ...form, roleId: selectedRole?.id, role: selectedRole?.name || '' }) }}>{roles.map(role => <option key={role.id} value={role.id}>{role.name}</option>)}</select></label>
      <label>{editing ? 'Contraseña' : <RequiredLabel>Contraseña</RequiredLabel>}<input required={!editing} minLength="8" autoComplete="new-password" type="password" value={form.password || ''} placeholder={editing ? 'Dejar vacío para conservarla' : 'Mínimo 8 caracteres'} onChange={set('password')} /></label>
      <div className="employee-technical-option">
        <label className="employee-technical-toggle">
          <input type="checkbox" checked={form.technicalEnabled === true} disabled={!canEnableTechnical} aria-describedby="employee-technical-help" onChange={event => setForm({ ...form, technicalEnabled: event.target.checked })} />
          <span>Puede realizar servicios técnicos</span>
        </label>
        <p id="employee-technical-help">Conserva su rol y agrega el acceso a Mis servicios. Solo Administración puede cambiar esta habilitación.</p>
      </div>
      <div className="employee-form-actions">
        <button className="primary"><Icon name="check" />{editing ? 'Guardar cambios' : 'Guardar empleado'}</button>
        <button type="button" className="secondary" onClick={cancel}><Icon name="close" size={16} />Cancelar</button>
      </div>
    </form>
  </EditorModal>
}
...
function Settings({ roles, setRoles, setNotice, ask }) {
  ...
  const save = () => {
    const nextId = editing === 'new' ? Date.now() : role.id
    const supervisorRole = normalizeRoleName(name) === 'supervisor'
    const next = {
      id: nextId,
      code: supervisorRole ? 'supervisor' : editing === 'new' ? `role-${nextId}` : roleCode(role),
      name,
      description,
      permissions: supervisorRole ? resolvedRolePermissions({ name: 'Supervisor' }) : editing === 'new' ? { ...DEFAULT_MODULE_PERMISSIONS, ...DEFAULT_FEATURE_PERMISSIONS, dashboard: true, agenda: true } : resolvedRolePermissions(role)
    }
    ask(editing === 'new' ? 'Crear rol' : 'Guardar permisos', `¿Querés confirmar los cambios del rol ${name}?`, () => { setRoles(prev => editing === 'new' ? [...prev, next] : prev.map(x => x.id === role.id ? next : x)); setActive(next.id); setEditing(false); setNotice('La configuración del rol fue guardada.') })
  }
  const toggle = key => {
    const current = resolvedRolePermissions(role)
    const enabled = Boolean(current[key])
    ask('Modificar permiso', `¿Querés ${enabled ? 'revocar' : 'otorgar'} “${[...MODULE_PERMISSIONS.map(item => [item[0], item[1]]), ...FEATURE_PERMISSIONS.map(item => [item[1], item[2]])].find(item => item[0] === key)?.[1] || key}” al rol ${role.name}?`, () => setRoles(previous => previous.map(item => {
      if (item.id !== role.id) return item
      const permissions = { ...resolvedRolePermissions(item), [key]: !enabled }
      if (enabled) FEATURE_PERMISSIONS.filter(([moduleKey]) => moduleKey === key).forEach(([, featureKey]) => { permissions[featureKey] = false })
      return { ...item, permissions }
    })))
  }
  return <><div className="module-intro"><div><p className="eyebrow">ADMINISTRACIÓN</p><h1>Roles y permisos</h1><p>Habilitá módulos completos y definí qué acciones puede realizar cada rol dentro de ellos.</p></div><button className="primary" onClick={() => { setName(''); setDescription(''); setEditing('new') }}><Icon name="plus" />Nuevo rol</button></div><div className="settings-grid"><article className="data-card roles-card"><h2>Roles disponibles</h2>{roles.map(r => <div className={r.id === role?.id ? 'selected-role' : ''} key={r.id} onClick={() => setActive(r.id)}><span className="role-dot">{r.name[0]}</span><div><b>{r.name}</b><p>{r.description}</p></div><button onClick={e => { e.stopPropagation(); startEdit(r) }} title="Editar rol"><Icon name="edit" size={16} /></button></div>)}</article><article className="data-card permissions-card">{editing ? <div className="role-editor"><p className="eyebrow">{editing === 'new' ? 'NUEVO ROL' : 'EDITAR ROL'}</p><label>Nombre del rol<input value={name} onChange={e => setName(e.target.value)} /></label><label>Descripción<input value={description} onChange={e => setDescription(e.target.value)} /></label><button className="primary" onClick={save}><Icon name="check" />Guardar rol</button><button className="secondary" onClick={() => setEditing(false)}><Icon name="close" size={16} />Cancelar</button></div> : <><p className="eyebrow">PERFIL: {role?.name?.toUpperCase()}</p><h2>Permisos por módulo y función</h2>{MODULE_PERMISSIONS.map(([key, label, detail]) => { const adminRole = roleCode(role) === 'administrator'; const supervisorRole = roleCode(role) === 'supervisor'; const auditOnly = key === 'audit'; const locked = auditOnly || adminRole || supervisorRole; const moduleEnabled = adminRole || Boolean(resolvedRolePermissions(role)[key]); const features = FEATURE_PERMISSIONS.filter(([moduleKey]) => moduleKey === key); const permissionDetail = supervisorRole ? key === 'history' ? 'Consulta de solo lectura limitada a cuentas con Servicio de CCTV' : 'No disponible para el rol Supervisor' : auditOnly ? 'Exclusivo del rol Administrador' : detail; return <section className="permission-group" key={key}><label className={`permission ${locked ? 'locked-permission' : ''}`}><span><b>{label}</b><small>{permissionDetail}</small></span><input type="checkbox" checked={moduleEnabled} disabled={locked} onChange={() => toggle(key)} /><i /></label>{features.length > 0 && <div className="feature-permissions">{features.map(([, featureKey, featureLabel, featureDetail]) => <label className={`permission feature-permission ${!moduleEnabled || adminRole || supervisorRole ? 'locked-permission' : ''}`} key={featureKey}><span><b>{featureLabel}</b><small>{supervisorRole ? 'No disponible para el rol Supervisor' : featureDetail}</small></span><input type="checkbox" checked={adminRole || Boolean(resolvedRolePermissions(role)[featureKey])} disabled={!moduleEnabled || adminRole || supervisorRole} onChange={() => toggle(featureKey)} /><i /></label>)}</div>}</section> })}<button className="primary save" onClick={() => startEdit(role)}><Icon name="edit" />Editar rol</button></>}</article></div></>
}

function ImportModal({ customers, close, onImport }) {
  const [message, setMessage] = useState('')
  ...
  const confirmImport = async () => {
    ...
    try { await onImport(preview.customers) } catch (error) { setMessage(error.message || 'No se pudo completar la importación.'); setSubmitting(false) }
  }

  return <div className="modal-layer"><div className="modal import-confirm-modal"><button className="close-modal" disabled={submitting} onClick={close}><Icon name="close" /></button><p className="eyebrow">IMPORTACIÓN MASIVA</p><h2>Importar abonados</h2><p>Seleccioná el reporte exportado. El archivo se analiza primero y la base solo cambia después de tu confirmación.</p><label className="file-drop"><Icon name="upload" size={30} /><b>Seleccioná un archivo .xls</b><small>Formato Maestro de Cuentas</small><input type="file" accept=".xls,.html" disabled={submitting} onChange={importFile} /></label>{preview && <div className="import-preview" role="status"><b>Revisá antes de confirmar</b><span>Archivo: {preview.fileName}</span><span>{preview.total} fila(s) válidas: {preview.created} abonado(s) nuevo(s) y {preview.updated} actualización(es).</span><small>Se guardará una copia de la base actual. Administración podrá deshacerla mientras no haya otra importación o modificación de clientes.</small></div>}{message && <p className="import-error">{message}</p>}<div className="modal-actions"><button className="secondary" disabled={submitting} onClick={close}><Icon name="close" size={16} />Cancelar</button><button className="primary" disabled={!preview || submitting} onClick={confirmImport}>{submitting ? 'Importando…' : 'Confirmar importación'}</button></div></div></div>
}
...
function ServiceConfirmationButton({ task, day, history = [], persist, onDraftChange, compact = false, disabled = false }) {
  ...
  const action = async () => {
    ...
    else throw new Error('Guardá primero el servicio para cambiar su confirmación.')
  }
  return <><button type="button" className={`service-confirmation-button ${awaiting ? 'awaiting' : ''} ${compact ? 'compact' : 'secondary'}`} disabled={disabled} title={disabled ? 'Guardá los cambios del servicio antes de cambiar la confirmación' : label} aria-label={label} onKeyDown={event => event.stopPropagation()} onClick={event => { event.stopPropagation(); setOpen(true) }}><Icon name={awaiting ? 'check' : 'clock'} size={15} />{!compact && <span>{label}</span>}</button>{open && createPortal(<div onClick={event => event.stopPropagation()} onKeyDown={event => event.stopPropagation()}><Confirm title={label} detail={`${awaiting ? 'El servicio quedará visible para los técnicos según su fecha y horario.' : 'El servicio reservará su horario, pero quedará oculto a los técnicos hasta confirmarlo.'}${onDraftChange ? ' El cambio se aplicará al guardar el servicio.' : ''}`} confirmLabel={label} action={action} close={() => setOpen(false)} /></div>, document.body)}</>
}
