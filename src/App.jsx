...
function HistoryManagementDetail({ record, setHistory, close, customers, services, employees, authUser, persistHistoryRecord, persistHistoryRecordRemoval }) {
  ...
  const [concurrentChange, setConcurrentChange] = useState(false)
  const interactionBlocked = concurrentChange || recordFingerprint !== acceptedFingerprintRef.current || saving
  useEffect(() => {
    const frame = requestAnimationFrame(() => {
