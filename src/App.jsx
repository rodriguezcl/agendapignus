...
function HistoryManagementDetail({ record, setHistory, close, customers, services, employees, authUser, persistHistoryRecord, persistHistoryRecordRemoval, historyScheduling }) {
  ...
  const saveChanges = () => {
    ...
    const patch = { ...draft, ...(record.vehicleControl ? { estimatedMinutes: 15, estimatedMinutesCustomized: false } : {}), ...customerLinkPatch(record, customer, authUser), serviceId: service.id, service: service.name, technicianIds: technicians.map(employee => employee.id), technicians: technicians.map(employee => employee.name) }
    if (!persistHistoryRecord) window.dispatchEvent(new CustomEvent('pignus:sync-agenda-service', { detail: { record, patch } }))
    update(patch)
  }
