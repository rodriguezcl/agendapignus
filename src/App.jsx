...
const blankVehicle = () => ({ brand: '', model: '', year: String(new Date().getFullYear()), plate: '' })
...
function Vehicles({ vehicles, setVehicles, setNotice, ask }) {
  ...
  const startEdit = vehicle => { setForm({ ...vehicle, year: String(vehicle.year) }); setEditing(vehicle.id); setOpen(true) }
  const save = event => {
    event.preventDefault()
    const record = { id: editing || globalThis.crypto?.randomUUID?.() || `vehicle-${Date.now()}`, brand: form.brand.trim(), model: form.model.trim(), year: Number(form.year), plate: form.plate.trim().toLocaleUpperCase('es-AR') }
    if (!record.brand || !record.model || !record.plate || !Number.isInteger(record.year) || record.year < 1886 || record.year > maximumYear) return setNotice(`Completá Marca, Modelo, Año y Matrícula. El año debe estar entre 1886 y ${maximumYear}.`)
