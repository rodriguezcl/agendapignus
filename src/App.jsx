...
function HistoryManagementDetail({ record, setHistory, close, customers, services, employees, authUser }) {
  ...
  const [draft, setDraft] = useState({ customerId: record.customerId || '', serviceId: record.serviceId || '', technicianIds: record.technicianIds || [], teamId: record.teamId || '', team: record.team || '', address: record.address || '', phone: record.phone || '', detail: record.detail || '', internalNote: record.internalNote || '', internalChecklist: normalizeInternalChecklist(record.internalChecklist) })
  const update = patch => {
