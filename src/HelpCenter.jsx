...
const MODULES = [
  ...
  },
  {
    ...
    articles: [
      ...
      { title: 'Servicios pendientes', body: 'El aviso amarillo contabiliza únicamente trabajos de hoy o de días anteriores que todavía necesitan una definición. Los servicios de mañana y fechas posteriores —incluidos los controles vehiculares programados— no se cuentan hasta que llegue su fecha. En una reprogramación se considera la nueva fecha asignada. Ingresá a Historial para completar, cancelar o reprogramar los trabajos vencidos.' }
    ]
  },
  {
    ...
    articles: [
      { title: 'Elegir la semana', body: 'Seleccioná una fecha en Semana de trabajo. La vista se posiciona en el día actual cuando corresponde y, desde el sábado a las 12:00, avanza automáticamente al lunes siguiente. Las barras superior e inferior permiten consultar días anteriores y posteriores. Las jornadas que ya finalizaron permanecen disponibles como consulta de solo lectura: no admiten servicios, equipos ni asignaciones nuevas. Cada trabajo reserva su tiempo estimado y no puede superponerse con otro del mismo equipo. Los trabajos breves conservan una reserva operativa mínima de 60 minutos; los turnos predeterminados vacíos que choquen con una franja ocupada dejan de mostrarse automáticamente.' },
      { title: 'Definir equipos y horarios del mes', body: 'Al comenzar un mes, un administrador debe confirmar la composición desde Equipos del mes y luego establecer dos turnos desde Horarios del mes. El sistema toma la cantidad activa de técnicos y vehículos para proponer equipos equilibrados. Con la dotación actual mantiene el ciclo de dos duplas y una salida individual; si la flota o el personal cambian, adapta la cantidad y el tamaño de los equipos y rota tanto compañeros como salidas individuales. La sugerencia puede modificarse antes de guardarla y no altera agendas ya cargadas. Cada modificación de los equipos conserva la composición anterior, la nueva, la fecha y el administrador responsable dentro de Historial de cambios. En meses futuros, los horarios rigen durante todo el mes. Si se cambian durante el mes actual, comienzan en la próxima jornada operativa y sólo ajustan espacios automáticos vacíos; los días anteriores, horarios manuales y servicios existentes conservan su hora. Los meses terminados quedan bloqueados.' },
      { title: 'Asignar responsables de los vehículos', body: 'Desde Vehículos del mes, asigná responsables a las unidades. La sugerencia se coordina con Equipos del mes: el Ford Ka queda a cargo de un técnico propuesto para salir solo; si hay varias salidas individuales, también alterna entre ellas según el historial. Los demás vehículos rotan responsables priorizando a quienes menos veces tuvieron esa unidad o responsabilidades anteriores. Si hay suficientes técnicos, cada vehículo recibe una persona diferente. El sistema genera automáticamente un control para cada viernes del mes a las 15:30 hs. Si una unidad no se utilizará esa semana, un administrador puede omitir su control desde la tarjeta semanal; dejará de mostrarse al técnico y no reaparecerá al guardar nuevamente la configuración mensual. Historial de cambios permite comparar las asignaciones anteriores y nuevas, incluso si luego se modifica la flota o el personal.' },
      ...
      { title: 'Agregar o editar un servicio', steps: ['Presioná Agregar servicio o seleccioná un horario disponible.', 'Elegí hora y tipo de servicio. La duración se completa con el valor del catálogo y podés ajustarla para ese trabajo particular.', 'Si elegís Instalación de alarma, indicá Docta, Nobu Town, Residencial con monitoreo o Residencial sin monitoreo. Esta última corresponde a clientes sin abono y no suma en las estadísticas de abonados.', 'Buscá el abonado o cliente por nombre, código o teléfono.', 'Sólo en Instalación de alarma, si será un abonado pero todavía no existe el PIG, presioná Reservar nuevo abonado y cargá nombre, dirección y contacto provisorios. La reserva ocupa el turno sin crear un CLI.', 'Cuando el PIG aparezca después de importar los datos, buscá la reserva y seleccioná ese PIG para vincularlo sin cambiar horario, equipo ni duración.', 'Usá Agregar cliente CLI solamente cuando sea un cliente real sin abono que deba quedar disponible para futuros servicios.', 'Completá el detalle y, si resulta útil para el técnico, adjuntá una foto de referencia desde la galería, un archivo de la computadora o la cámara.', 'Guardá el servicio.'], note: 'Los servicios se ordenan automáticamente por horario y el sistema impide que sus franjas ocupadas se superpongan dentro del mismo equipo.' },
      { title: 'Adjuntar una foto de referencia', body: 'En el formulario del servicio, usá Galería o archivo para elegir una imagen existente o Sacar foto para utilizar la cámara. Se admite una foto JPG, PNG o WebP de hasta 1 MB. La imagen queda disponible para Administración y usuarios en Agenda semanal, Agenda del día e Historial, y para el equipo técnico asignado. También podés reemplazarla o eliminarla desde el mismo servicio.' },
      { title: 'Diferenciar planificación y tiempos reales', body: 'Existe un conflicto de agenda solamente cuando los horarios y tiempos estimados cargados por Administración se superponen. Si un servicio de hoy termina antes, el equipo queda disponible desde el siguiente cuarto de hora y podés aprovechar esa ventana siempre que el nuevo trabajo termine antes del próximo servicio programado. Si termina después de lo previsto, la demora queda registrada como una contingencia operativa, pero no prolonga la franja planificada ni genera un conflicto rojo.' },
      { title: 'Guardar servicios cargados', body: 'Completá el formulario y presioná Guardar servicio. Esperá la confirmación: ese servicio se registra inmediatamente como pendiente en Agenda semanal, Agenda del día e Historial, incluso para fechas futuras. No es necesario guardar la columna del día. Cada operación modifica sólo los registros afectados; si otra sesión cambió el mismo servicio o existe un conflicto de horarios, se rechaza el guardado y el formulario permanece abierto para revisar los datos.' },
      { title: 'Campos adicionales según el servicio', body: 'Forma de pago, Monto, Abono mensual y Formulario se habilitan solamente cuando corresponden al tipo de servicio. Forma de pago permite Efectivo, Transferencia, Débito, Crédito, A confirmar o No aplica. Monto aparece al elegir una forma de pago, se muestra con el signo $ y separadores de miles, y es obligatorio salvo con A confirmar; con No aplica no se solicita. En Instalación de alarma, el abono mensual se habilita únicamente para una instalación Residencial con monitoreo y también se muestra con el signo $ y separadores de miles. Formulario ofrece Completo o Incompleto (Abonado completa a mano), tanto en Instalación de alarma como en Cambio de titularidad.' },
      { title: 'Eliminar o reasignar un servicio', body: 'Usá el botón rojo del servicio para eliminarlo. El botón de traslado permite elegir otro equipo del mismo día o una fecha diferente. Si cambiás la fecha, el servicio vuelve a estado Pendiente como una reprogramación. En ambos casos se conservan sus datos y Agenda semanal, Agenda del día e Historial se actualizan juntos.' },
      { title: 'Guardias de los sábados', body: 'Desde Guardias del año, un administrador define el orden de rotación de los técnicos. El ciclo comienza el primer sábado y se repite automáticamente durante todo el año. Cada cambio del orden anual conserva la rotación anterior, la nueva, la fecha y el administrador responsable en Historial de cambios. Cada sábado puede corregirse manualmente como excepción sin alterar el orden general. Si el técnico de guardia recibe un servicio el viernes entre las 16:00 y las 20:00, el sistema interpreta que adelantó su guardia: bloquea cualquier nuevo equipo, técnico o servicio del sábado y muestra una leyenda informativa.' },
  ...
  },
  {
    ...
    articles: [
      { title: 'Cargar la agenda de una fecha', body: 'Elegí la fecha de trabajo. La agenda carga la planificación semanal y los servicios reprogramados de ese día. Podés agregar trabajos de último momento sin perder los ya registrados mientras la jornada no haya finalizado. Las fechas pasadas quedan bloqueadas en las agendas y se consultan o corrigen desde Historial. Los domingos permanecen bloqueados por ser días no operativos. Si es feriado, primero debe existir una definición de Día laboral; cuando se define como Día no operativo tampoco admite equipos, técnicos ni servicios.' },
      { title: 'Completar un servicio', steps: ['Ingresá la hora.', 'Seleccioná el tipo de servicio.', 'Buscá y elegí el abonado o cliente.', 'Revisá dirección, contacto y observaciones.', 'Si es una instalación de alarma, elegí la ubicación y completá los datos adicionales habilitados.', 'Si ayuda a identificar el trabajo, adjuntá una foto de referencia desde un archivo, la galería o la cámara.'], note: 'En Vista previa, Copiar agenda y la agenda técnica sólo se informa la forma de pago cuando es Efectivo —junto con el monto— y el Formulario cuando está Incompleto. El abono mensual y las demás opciones quedan para la gestión administrativa.' },
      { title: 'Reserva PIG o cliente nuevo', body: 'Reservar nuevo abonado se habilita únicamente al seleccionar Instalación de alarma. Si será un nuevo abonado y todavía no fue creado en SoftGuard, cargá datos provisorios y vinculá después el PIG importado: no se crea ningún CLI residual y el turno conserva su identidad. Dos personas pueden tener el mismo nombre; una coincidencia por nombre no vincula automáticamente una cuenta existente. Usá Agregar cliente CLI sólo para una persona que realmente será cliente sin abono. Para una cuenta existente, corregí sus datos desde Abonados y clientes.' },
  ...
  },
  {
    ...
    articles: [
      ...
      { title: 'Gestionar un servicio', body: 'Presioná Gestionar para ver sus datos y la foto de referencia, si fue adjuntada. Desde allí podés cargar, reemplazar o eliminar esa foto, marcar el servicio como completado, devolverlo a pendiente, cancelarlo, reprogramarlo, editar información permitida o eliminar el registro. El rol Administrador puede marcarlo como completado aunque todavía no haya llegado su horario; la agenda del técnico continúa respetando la fecha y hora programadas.' },
  ...
  },
  {
    ...
    articles: [
      ...
      { title: 'Seguimiento de Servicio de CCTV', body: 'Al editar un abonado o cliente, activá Servicio de CCTV para incluir su historial en el seguimiento de Supervisión. El rol Supervisor sólo puede consultar los servicios de las cuentas marcadas y ve las observaciones informadas por los técnicos. La selección se conserva cuando volvés a importar la base desde SoftGuard.' },
  ...
  },
  {
    ...
    articles: [
      { title: 'Servicios asignados', body: 'Agenda técnica muestra todos los servicios vencidos que sigan pendientes, además de los trabajos de hoy y mañana, evitando anticipar el resto del mes. Los trabajos en domicilio deben resolverse en el orden indicado: la dirección y el contacto del siguiente se habilitan después de informar el estado del servicio actual. Los controles vehiculares son tareas autónomas: no poseen dirección ni contacto y se habilitan desde su propia fecha y hora. Si el viernes es un feriado definido como no operativo, el control pasa automáticamente al último día operativo anterior, incluso ante feriados consecutivos. Un control vehicular vencido permanece visible y disponible para cargar foto y kilometraje; los demás trabajos pueden consultarse, pero sus domicilios y contactos permanecen ocultos hasta completarlo.' },
      { title: 'Marcar un servicio como completado', steps: ['Presioná Marcar completado.', 'Cargá la observación técnica obligatoria con el trabajo realizado y cualquier dato útil para futuras visitas.', 'Presioná Continuar.', 'Revisá el mensaje de confirmación y elegí Sí, confirmar estado.'], note: 'Puede completarse desde su fecha y hora programadas o posteriormente si quedó vencido. Al confirmar se registra la hora real y, si terminó antes de lo previsto, la agenda libera al equipo desde el siguiente cuarto de hora. La observación también es obligatoria al solicitar una reprogramación o informar una cancelación. El control vehicular es la excepción: requiere foto interior y kilometraje, pero su observación es opcional.' },
      { title: 'Consultar la foto de referencia', body: 'Si Administración adjuntó una imagen al servicio, aparece debajo del nombre del cliente. Tocala para verla en tamaño completo. La foto sólo está disponible para los técnicos asignados a ese trabajo y sirve como apoyo para reconocer el lugar, el equipo o la tarea antes de la visita.' },
  ...
  },
  {
    ...
    articles: [
      ...
      { title: 'Tiempo estimado por servicio', body: 'Cada tipo de servicio posee una duración predeterminada expresada en horas y minutos. Cuando cambia ese valor, también se actualizan los servicios pendientes que todavía utilizaban el predeterminado anterior. Un tiempo ajustado manualmente por cantidad de equipos, cableado u otra complejidad se conserva únicamente para ese trabajo.' },
  ...
  },
  {
    ...
    articles: [
      ...
      { title: 'Cargar y consultar el seguro', body: 'El administrador puede adjuntar el seguro vigente en PDF —hasta 3 MB— e indicar su fecha de vencimiento. El archivo queda protegido por la sesión. Los técnicos lo descargan desde Vehículos y seguros; cuando vence, el Menú principal identifica la unidad y solicita gestionar el PDF actualizado con el área administrativa.' },
      { title: 'Control semanal de limpieza y kilometraje', body: 'El técnico responsable recibe los viernes a las 15:30 un servicio interno de 15 minutos. Puede realizarlo antes de esa hora cuando ya haya finalizado todos los demás servicios programados para ese día. No requiere abonado, cliente, reserva PIG, dirección, contacto ni observación: para completarlo debe informar un kilometraje superior al último registrado y adjuntar una foto del interior. El responsable se define para todo el mes, pero desde Vehículos del mes puede elegirse un reemplazo para un viernes puntual ante una contingencia. Si el vehículo no se utilizará esa semana, un administrador puede omitir el control desde Agenda semanal y el técnico no tendrá que realizarlo. Al confirmar un control vigente, el kilometraje del vehículo se actualiza automáticamente y Administración puede consultar la foto desde el detalle del Historial. Si queda vencido, bloquea el domicilio y el contacto de la agenda siguiente —incluidos sábado y lunes— hasta que sea completado; no puede omitirse cancelándolo o solicitando una reprogramación.' },
...
const FAQ = [
  ...
  ['¿Qué hago si la fecha es domingo o feriado?', 'Los domingos permanecen bloqueados y no admiten servicios. En un feriado, un administrador debe elegir Día laboral para habilitar la agenda o Día no operativo para bloquearla. Los demás perfiles pueden ver la decisión, pero no modificarla.'],
...
const TECHNICIAN_FAQ = [
  ['¿Por qué no veo la dirección del próximo servicio?', 'Los trabajos en domicilio se habilitan en orden. Informá primero el estado del servicio anterior para acceder a la dirección y el contacto del siguiente.'],
  ['¿Cuándo puedo completar un servicio?', 'Desde la fecha y hora programadas. Si quedó vencido, seguirá visible hasta que informes su estado.'],
  ['¿La observación técnica es obligatoria?', 'Sí, para completar, cancelar o solicitar la reprogramación de un servicio. En el control vehicular es opcional porque se exigen la foto interior y el kilometraje actualizado.'],
  ['¿Dónde veo la foto de referencia?', 'Cuando el servicio tiene una foto adjunta, aparece debajo del nombre del cliente. Tocala para abrirla en tamaño completo.'],
  ['¿Cómo consulto trabajos anteriores?', 'Abrí Historial y buscá por cliente, código, servicio, fecha o detalle. Allí aparecen los servicios cuyo estado ya informaste.'],
  ['¿Dónde consulto antecedentes del cliente?', 'Dentro de un servicio habilitado, presioná Ver historial del cliente. La información es de solo lectura.'],
  ['¿Cómo descargo el seguro de un vehículo?', 'Abrí Vehículos y presioná Descargar PDF en la unidad correspondiente. Si figura No disponible, solicitá el documento actualizado a Administración.'],
  ['¿Qué hago si estoy sin conexión?', 'La agenda visible se conserva, pero esperá a recuperar conexión antes de informar un estado para asegurar que el cambio se guarde.']
]

const TECHNICIAN_MODULE_IDS = new Set(['technician', 'vehicles', 'mobile'])
...
export function HelpShell({ user, onNavigate, logout, theme, setTheme, isAdministrator, navigation }) {
  ...
  return <div className="app-shell" data-theme={theme}>
    ...
    <main><header className="topbar"><button className="mobile-menu" onClick={() => setMenuOpen(true)}><Icon name="menu" /></button><div className="page-heading"><span>PIGNUS</span><i /><b>Centro de ayuda</b></div><div className="profile"><button className="theme-toggle" onClick={() => setTheme(theme === 'light' ? 'dark' : 'light')} title={theme === 'light' ? 'Activar modo oscuro' : 'Activar modo claro'}><Icon name={theme === 'light' ? 'moon' : 'sun'} /></button><span className="profile-avatar">{String(user?.name || 'U').split(' ').map(part => part[0]).slice(0, 2).join('').toUpperCase()}</span><span>{user?.name}</span><button className="logout-button" onClick={logout}><Icon name="logout" size={17} /><span>Cerrar sesión</span></button></div></header><section className="content"><HelpCenter onNavigate={onNavigate} isAdministrator={isAdministrator} allowedModules={navigation.map(([id]) => id)} /></section></main>
  </div>
}

export function TechnicianHelpCenter({ onNavigate }) {
  return <HelpCenter onNavigate={onNavigate} technicianOnly />
}

function HelpCenter({ onNavigate, isAdministrator = false, allowedModules = [], technicianOnly = false }) {
  const allowedModuleKey = allowedModules.join('|')
  const available = useMemo(() => MODULES.filter(item => technicianOnly
    ? TECHNICIAN_MODULE_IDS.has(item.id)
    : (isAdministrator || !item.adminOnly) && (!item.module || allowedModules.includes(item.module))), [isAdministrator, allowedModuleKey, technicianOnly])
  const faqEntries = technicianOnly ? TECHNICIAN_FAQ : FAQ
  const [query, setQuery] = useState('')
  const [active, setActive] = useState(technicianOnly ? 'technician' : 'start')
  const [open, setOpen] = useState(technicianOnly ? 'technician-0' : 'start-0')
  ...
  return <div className="help-center">
    <header className="help-hero">
      <div><p className="eyebrow">GUÍAS Y PREGUNTAS FRECUENTES</p><h1>Centro de ayuda</h1><span className="help-updated">Actualizado · 15 de septiembre de 2026</span><p>{technicianOnly ? 'Consultá las funciones disponibles en tu perfil técnico.' : 'Encontrá instrucciones simples para completar cada tarea con seguridad.'}</p></div>
    ...
    {query ? <section className="help-results data-card"><div className="help-section-title"><div><span>{searchResults.length}</span><h2>Resultados para “{query}”</h2></div></div>{searchResults.length ? searchResults.map(result => <button className="help-result" key={`${result.section.id}-${result.index}`} onClick={() => choose(result.section.id, result.index)}><Icon name={result.section.icon} /><span><b>{result.title}</b><small>{result.section.title}</small></span><i>Ver respuesta →</i></button>) : <div className="help-empty"><Icon name="search" size={30} /><h3>No encontramos una respuesta con esas palabras</h3><p>Probá con términos como “feriado”, “guardia”, “cliente”, “técnico” o “copiar”.</p></div>}</section> : <div className="help-layout">
      ...
      <section className="help-content data-card">{active === 'faq' ? <><div className="help-section-title"><div><span><Icon name="help" /></span><div><p className="eyebrow">RESPUESTAS RÁPIDAS</p><h2>Preguntas frecuentes</h2></div></div></div><div className="help-articles">{faqEntries.map(([question, answer], index) => <HelpArticle key={question} id={`faq-${index}`} title={question} body={answer} open={open} setOpen={setOpen} />)}</div></> : <><div className="help-section-title"><div><span><Icon name={selected.icon} /></span><div><p className="eyebrow">GUÍA DEL MÓDULO</p><h2>{selected.title}</h2><p>{selected.summary}</p></div></div>{selected.module && <button className="secondary" onClick={() => onNavigate(selected.module)}>Ir al módulo <span>→</span></button>}</div><div className="help-articles">{visibleArticles.map((article, index) => <HelpArticle key={article.title} id={`${selected.id}-${index}`} {...article} open={open} setOpen={setOpen} />)}</div></>}</section>
    </div>}
    <section className="help-tip"><span>i</span><div><b>Consejo</b><p>{technicianOnly ? 'Antes de confirmar un estado, revisá el servicio y la observación: el cambio queda registrado con tu nombre y horario.' : 'Si una acción puede modificar o eliminar información, leé el mensaje de confirmación antes de continuar. El sistema explica qué datos serán afectados.'}</p></div></section>
  </div>
}
