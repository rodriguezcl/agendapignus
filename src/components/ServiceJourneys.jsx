...
export function ServiceJourneys({ record, disabled = false }) {
  const context = useContext(ServiceJourneysContext)
  const titleId = useId()
  ...
  const close = () => { if (!saving && window.confirm('¿Descartar la planificación de jornadas sin guardar?')) setOpen(false) }
  ...
  if (record.serviceJourney) return <small className="journey-label">{journeyLabel(record)}</small>
  ...
  return <><button ref={openerRef} type="button" disabled={disabled} title={disabled ? 'Guardá los cambios antes de planificar las jornadas.' : undefined} className="link-button journey-plan-button" onKeyDown={event => event.stopPropagation()} onClick={event => {
    event.stopPropagation(); setVisits([{ date: record.date, time: record.time, teamId: record.teamId, estimatedMinutes: record.estimatedMinutes }, { date: '', time: '', teamId: '', estimatedMinutes: 60 }]); setError(''); setOpen(true)
  }}>Planificar varias jornadas</button>{open && createPortal(<div className="modal-layer journey-layer" onClick={event => event.stopPropagation()} onKeyDown={dialogKeyDown}><section ref={dialogRef} tabIndex={-1} className="modal journey-modal" role="dialog" aria-modal="true" aria-labelledby={titleId}><h2 id={titleId}>Servicio en varias jornadas</h2><p>{record.client}</p><p>La primera visita conserva su fecha, hora y equipo. Ajustá su duración y reservá las siguientes. Se guardan todas juntas; si hay un conflicto no se guarda ninguna.</p><form onSubmit={submit}><fieldset disabled={saving}>{visits.map((visit,index) => {
