...
export function ServiceJourneys({ record, disabled = false, compact = false }) {
  const context = useContext(ServiceJourneysContext)
  const titleId = useId()
  ...
  if (record.serviceJourney) return compact ? null : <small className="journey-label">{journeyLabel(record)}</small>
