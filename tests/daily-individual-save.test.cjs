...
test('saving from dialog marks the target card and cancellation preserves saved services', () => {
  ...
  assert.match(daily, /if \(task.taskId !== first.task.taskId\) return task/)
  assert.match(daily, /saved \? restoreSavedService\(task, saved\)/)
...
test('module navigation waits for successful saves and excludes empty cards', () => {
  assert.match(daily, /taskHasContent\(task\) && dailyServiceHasChanges\(task\)/)
